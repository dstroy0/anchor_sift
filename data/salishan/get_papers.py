...
#
#   python data/salishan/get_papers.py             the papers the readers need
#   python data/salishan/get_papers.py --all       every paper in the archive
#   python data/salishan/get_papers.py --convert   convert PDFs already on disk
#   python data/salishan/get_papers.py --list      print what would be fetched
#
...
# interleaved.
#
# pypdf reads a page in the order the PDF stores it, the order the readers were written
...
def converted(source, target):
    ...
    if unmapped:
        with open(notice, "w", encoding="utf-8", newline="\n") as handle:
            ...
            handle.write("  python data/salishan/pdf2png.py %s 1 <last>\n\n"
...
def report_unfaithful(out, stems):
    ...
    for stem in stems:
        out.write("    python data/salishan/pdf2png.py %s 1 <last>\n" % stem)
