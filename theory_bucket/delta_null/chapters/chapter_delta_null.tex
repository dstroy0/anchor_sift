...
\textbf{Purpose:} Determine what identity gains in information versus what it takes to compute. \textbf{Test scope:} \texttt{test/\allowbreak{}bench/\allowbreak{}}, \texttt{test/\allowbreak{}support/\allowbreak{}}, \texttt{src/\allowbreak{}impensa\_\allowbreak{}ancorae\_\allowbreak{}acus/\allowbreak{}}
...
Many different applications and experimental apparatus are available for use in the repository this
body of work came from. See \texttt{examples/\allowbreak{}} for application and
\texttt{src/\allowbreak{}} for engines and engine sims.
...
region or crosses the boundary into the rejected exterior. This is the survivorship bias in the
algorithm. The contrapositive \(\neg\,\mathrm{Anc}_A(s) \Longrightarrow \neg\,\mathrm{Occ}(s)\) means
...
flux of rejected candidates passing through the filter per operation.
...
\noindent\textbf{The selection rule is free.} Proposition 1 quantifies over every subconfiguration and never mentions what chose one. No frequency table, no random table, and no absence of a table can produce a false negative. What a measure moves is how often the rejection fires.

\noindent\textbf{The alphabet need not be known.} Both predicates are equalities between positions. Where \(\Sigma\) cannot be enumerated no frequency table exists, and the flat measure is what remains. Maximum entropy is therefore the base case for this construction, and a weighted table is the special case available on a known domain.
...
\noindent\textbf{Figure 1.} The anchor filter, as \texttt{bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c:193-\allowbreak{}239} implements it. Refuted is the only box the method reaches with certainty. Everything below the candidate is work.
...
The examples under \texttt{examples/\allowbreak{}} are 122 scripts over seven domains, each one a stage of the same pipeline, and every domain result in the other theories comes from one of them. Appendix B lists the stages and what each domain reaches.
...
subconfiguration. A row that reports a refusal is a row to fix, not a finding.
...
(\texttt{test/\allowbreak{}bench/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:715-\allowbreak{}757}).
...
The quantity that sets the candidate rate in Section 4.4 sets the contrapositive indication distance here, appearing once as a rate and once as a length. Measured at every needle length over every corpus (\texttt{test/\allowbreak{}bench/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:1024}):
...
The examples under \texttt{examples/\allowbreak{}} are 122 scripts over seven domains, each one a
...
\texttt{art}             &  6 & measure \\
\texttt{crystallography} &  6 & oracle \\
\texttt{proteins}        &  4 & sift \\
\texttt{source}          &  4 & measure \\
\texttt{sound}           &  1 & represent \\
\bottomrule
\end{tabular}
\end{center}

Two domains reach an oracle and the rest stop short of one. Crystallography has published cell
edges and language has hand extractions, and those are the only two places an answer exists that
this work did not produce. Everything else is measured against its own null permutation and says
so.
