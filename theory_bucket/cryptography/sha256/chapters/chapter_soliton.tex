...
The projection sums over the output axis. It cannot say which output bits these are. Naming them
...
because carries only run upward. Bit \(k\) of a message word drives bit \(k\) of \(T_1\) and
...
at source bit twenty that is \(k - 11 = 9\), below \(k - 25 = 27\). Residue eleven should appear.
It never does.
...
A soliton of probability-one paths is what a local-collision construction is built out of. This
