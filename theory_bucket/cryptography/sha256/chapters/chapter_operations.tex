...
The suspicion was that the schedule pair should be lossy where the state pair is not, since \(\sigma_0\) and \(\sigma_1\) each carry a shift and a shift discards bits where a rotation cannot. All four are \(\mathrm{GF}(2)\)-linear. This needs no sampling: each is a \(32 \times 32\) matrix and its image is two to its rank.
...
Every one is a bijection. Exclusive-or against two rotations restores what the shift drops. The schedule pair collides no earlier than the state pair. Handschuh and Gilbert state this for the \(\Sigma\) pair; the measurement here reproduces it for all four independently.

\section{The narrowed rank deficit in closed form}

Narrowed, they do lose rank, and the deficit is exact. A rotation right by \(a\) on a \(w\)-bit word is multiplication by \(x^{w-a}\) in \(\mathrm{GF}(2)[x]/(x^w + 1)\). Three rotations exclusive-ored is multiplication by a three-term polynomial and the kernel is
\[
...
Degree is proved, not sampled. A Boolean function of degree below \(d\) has every \(d\)-th order derivative identically zero, and that derivative is the exclusive-or of the output over all corners of a \(d\)-dimensional input cube. One nonzero fold settles it.
...
The \(\geq 18\) entries are the cube ceiling of the bench. No measurement produced them. They establish the separation and say nothing about the carry's absolute degree.
...
Removing addition alone still reaches the same diffusion plateau as the full function. The carry is not what makes diffusion happen; the nonlinearity of choice and majority is, and the carry changes how fast, not whether. That matches the published finding that removing addition costs about two rounds and does not stop diffusion.
...
Choice and majority act inside the bit lattice, where \(\mathrm{GF}(2)\) describes them. A carry leaves its bit position, travels where \(\mathrm{GF}(2)\) has no coordinate, and rejoins one position higher. The measurement says that path outside the lattice carries all but a negligible part of the degree.

It is also why a bijection can appear to destroy information while destroying none. The round stays invertible and the difference stays exact; a \(\mathrm{GF}(2)\)-shaped instrument simply loses the ability to describe it and reports noise. Remove the extradimensional path and nothing ever becomes indescribable. The linear variant, pinned at \(0.5\) for all sixty-four rounds, shows that.
