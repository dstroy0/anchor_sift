...
\section{Anchor-sift on Salishan}

\textbf{Purpose:} Apply the anchor-sift method to the Salishan papers: find the language, and decide what may join a pure corpus. \textbf{Scope:} \texttt{data/\allowbreak{}salishan/\allowbreak{}}

The method is in \texttt{.\allowbreak{}.\allowbreak{}/\allowbreak{}anchor-\allowbreak{}sift-\allowbreak{}method.\allowbreak{}md}. This is what happens when it is fed these papers.
...
Directory & Holds \\
\midrule
\texttt{anchor\_\allowbreak{}sift/\allowbreak{}} & the algorithm: squash, total variation, split-half, support and entropy \\
\texttt{corpus\_\allowbreak{}script\_\allowbreak{}extraction/\allowbreak{}} & the twelve hand-written readers and their repairs and checks \\
\texttt{anchor\_\allowbreak{}sift\_\allowbreak{}algorithmic\_\allowbreak{}extraction/\allowbreak{}} & the sift applied to the papers with no reader, and \texttt{boundary\_\allowbreak{}check.\allowbreak{}py} \\
\texttt{word\_\allowbreak{}web/\allowbreak{}} & the forms of every hand extraction joined by concept, shape and context \\
\bottomrule
\end{longtable}
\normalsize

\texttt{anchor\_\allowbreak{}sift.\allowbreak{}py} has nothing to tune and no per-language term. Everything else decides what to hand it.
...
The thirteenth, \texttt{1975\_\allowbreak{}Hilbert\_\allowbreak{}Hess}, is read and has no reader yet. \texttt{reader\_\allowbreak{}check} holds it ungraded instead of counting it. The six languages are the ones with a pure corpus on disk; two of the thirteen papers are in languages that have no anchor.
...
\toprule
Language & Pure & Candidates & Admitted & D\_self before & after & Support after \\
\midrule
...
Growing the Lushootseed anchor does not reach the 103 papers, and an earlier version of this document said it would. The 95 that are large enough to ask fail because a paper's residue sits almost the same distance from all six language anchors: the median gap between the nearest and the runner-up is 0.0247 and the median paper floor is 0.3527. Nothing about the anchors' own sizes moves those two numbers past each other.

What the 95 need is a measure that separates the anchors as seen from a mixed residue, which the byte pair distribution does not. \texttt{corpus-\allowbreak{}derivation.\allowbreak{}md} section 7 has the first thing that does: holding a concept fixed with the word web's gloss edge, flattening the pooled counts to maximum entropy, and testing one run at a time. That recovers a published dialect border inside Lushootseed at a corpus size where the whole-distribution comparison needs 2.9 times more text. It has not yet been turned on the 95.
