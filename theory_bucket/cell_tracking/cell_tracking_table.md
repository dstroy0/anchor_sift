...
| **15. The whole state, once** | The field's state is a set of bodies at every time, and one exact file holds all of it. | The machine holds the field (M9: `.iapx`, `.oapx`, `flatten`, `.bapx`). No file holds the bodies' fingerprints. | A file kind for the bodies and their ℓ ≤ 2 fingerprints, written once; linking reads bodies and not voxels. | See M9. | field held by the machine; bodies' prints not held | With row 2. |
