...
The relation found is not unique, and an early version of this control demanded that it be. Bailey's compendium notes that the auxiliary sums satisfy zero relations among themselves. The exact relations form a lattice and the known formula is one member of it. Demanding a particular member was the wrong control, and it reported a correct answer as a failure.
...
4{,}400 & \(145.57\) & \(6{,}671{,}400\) & \(2{,}825\) & \textemdash{} \\
\bottomrule
\end{tabular}
\end{center}
...
The multiply is what carries it. The mean operand width grows \(1.89\), \(1.95\), \(1.99\) per doubling. It is linear in the precision to within the resolution of four rows. Karatsuba costs \(\mathrm{width}^{1.585}\), and a linear width against a roughly linear count of multiplies predicts an exponent of \(1 + 1.585 = 2.585\). The measurement returned \(2.64\).
