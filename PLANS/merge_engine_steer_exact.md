...
Local `main` is a different thing from `origin/main` and is not the target: it sits at `87950d3`, behind `origin/main` by 29, and it is checked out in the primary worktree at `D:/git_project/repos/owned/public/anchor_sift`. It cannot be merged into from here.
...
It carries a second entry, `anchor_steer_spawn_coarms_deep`, that exists only to run the descent without the destroy rule. This branch replaced it with the `force_full_depth` member. One descent serves both and the two cannot drift.

It has no `AnchorSameAt`, no `AnchorField` and no `anchor_field_project`. It searches bytes and nothing else. The any-type oracle and the rarity projection are only on this branch.
