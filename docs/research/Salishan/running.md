...
python tools/Salishan/get_papers.py
```
...
```
tools/Salishan/get_papers.py                         the archive
tools/Salishan/pdf2png.py                            the page as an image
tools/Salishan/draft_page_text.py                    the page as a first draft
tools/Salishan/hand_extraction/                      the control
tools/Salishan/anchor_sift/                          the algorithm
tools/Salishan/corpus_script_extraction/             the eleven readers
tools/Salishan/anchor_sift_algorithmic_extraction/   the sift applied
tools/Salishan/sound_representation/                 the recordings as bits
```
...
One per paper. Each takes no arguments and rebuilds that paper's corpus. Written `S/` for `tools/Salishan/corpus_script_extraction/`.
...
A reader is not the source of the corpus. The hand extraction is, and the reader is graded against it. Written `H/` for `tools/Salishan/hand_extraction/`.

```
python H/oracle_check.py
```

Checks each hand extraction against the paper it was read off, in both directions. A form written down that the paper does not hold is a typing slip. A word in the paper that no row holds is a row somebody skipped, and a person reading a thirty-seven page paper into a table actually produces them. Both must be zero.
...
Checks each reader against the hand extraction. Grades whether it found each form, whether it says the same kind, and whether it says the same dialect. Kind is the grade that decides the corpus. A rejected tableau candidate filed as a citation puts a form the paper's own analysis rejects into the pure stream, and nothing downstream asks again.

## When the text is not the page

The `build/papers/*.txt` for `19-Lyon_ICSNL50_final-78` and `2013_Lindley_Lyon` holds the font's alphabet and not what the page prints, because both are set in a font that renumbers its glyph codes and declares no map back to Unicode. `refs.md` has the detail, the other four papers in the same class, and why the damaged text is kept.

```
python tools/Salishan/pdf2png.py 2013_Lindley_Lyon 1 70
python tools/Salishan/draft_page_text.py 2013_Lindley_Lyon
```
...
Written `A/` for `tools/Salishan/anchor_sift/` and `X/` for `tools/Salishan/anchor_sift_algorithmic_extraction/`.
...
The first reads which language each paper is about out of its own title and abstract. The second sorts every paper without a reader against both anchors and writes `build/corpora/sifted/`. The third asks whether the distributions agree with what each paper says it is about, with section 3 deciding whether the reading may be made. The last two print each corpus's split-half distance, support and entropy as it grows, and admit candidates in batches while the corpus stays on that curve.

## The sound representation

This one does not read `build/papers/` and nothing in the text pipeline depends on it. The script reads the recordings.

```
python tools/Salishan/sound_representation/binary_sound.py
python tools/Salishan/sound_representation/binary_sound.py <stem>
```
...
`TEXT_SPACE` is the only definition of what characters these orthographies are written with. `salish_unsorted.py` and `H/papers.py` each held their own copy of that union until the copies were noticed; both read it from `salish_marking.py` now, and a per-paper set is written as that plus what the paper adds.
