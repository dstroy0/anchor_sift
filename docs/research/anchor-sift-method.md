...
**Scope:** `examples/language/4_measure/byte_signature.py`, `tools/Salishan/corpus_script_extraction/salish_purity.py`, `tools/instrument/corpus_gate.py`, `examples/language/4_measure/case_or_splitting.py`, `examples/language/4_measure/cluster_profiles.py`, `examples/language/4_measure/evidential_pressure.py`
...
$P_T$ is a probability distribution over $2^{16}$ outcomes (`examples/language/4_measure/byte_signature.py:85-93`).

## 2. Total Variation

$$D(P,Q) \;=\; \tfrac{1}{2} \sum_{k=0}^{65535} \bigl| P(k) - Q(k) \bigr| \;\in\; [0,1]$$

Total variation. $D=0$ when the distributions agree everywhere, $D=1$ when their supports are disjoint (`examples/language/4_measure/byte_signature.py:96-99`).

## 3. Split-Half Resolution
...
and is uninterpretable when $D_{\text{self}}(T) > \max_{T'} D_{\text{self}}(T')$, which says the corpus is too small to distinguish from noise (`examples/language/4_measure/byte_signature.py:162-196`).

Measured on 20 corpora cut to 6707 bytes each: $D_{\text{self}}$ ranged $0.1118$ to $0.3192$, mean $0.2361$.

## 4. Support and Entropy Correction
...
Across those 20 corpora, $\mathrm{supp}$ ran $193$ to $970$ and tracked $D_{\text{self}}$ almost monotonically. A corpus in a low-support encoding looks internally consistent for arithmetic reasons. Any ranking of $D_{\text{self}}$ across different writing systems is a ranking of $\mathrm{supp}$ until shown otherwise (`examples/language/4_measure/byte_signature.py:157-175`).

## 5. Extraction Corruption
...
where $T_\alpha$ is the alphabetic subsequence and $\mathrm{comb}$ marks combining characters. Extraction loss drops combining marks first, so $\mu \to 0$ with $\mathrm{sets} \ll q$ identifies a corrupted file whose surviving text is still well-formed (`tools/Salishan/corpus_script_extraction/salish_purity.py:57-71`).
...
$\Delta \le 0$ means the observed split bought nothing a random split of identical shape would not (`examples/language/4_measure/case_or_splitting.py:110-131`). Over 19 corpora capped to 60000 tokens, $\Delta$ ranged $+0.0056$ to $+0.0901$, with null scatter $\le 0.0133$ over 5 draws.

## 7. Cophenetic Validation
...
Low $\rho_c$ means the tree imposes structure the distances do not carry, and the output is an ordering with no groups in it (`examples/language/4_measure/cluster_profiles.py`). Measured $\rho_c = 0.8406$ over $171$ pairs on 19 languages.

## 8. Family-Stratified Sampling
...
over $d$ draws and family set $F$. Report $\hat{\pi}$ with its across-draw standard deviation (`examples/language/4_measure/evidential_pressure.py:65-78`). Run on 418 languages across 121 families, $d=300$: area rates $0.089$ to $0.716$ with scatter $0.029$ to $0.069$.
