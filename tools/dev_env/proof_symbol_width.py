...
# real, it lives at the unit width, and nothing was placed at any other. Carving at the right width
# should show it and carving at any other should not.
#
# The phase is swept as well. A detector is not told where the units begin, and a carving that is the
...
def main():
    ...
    out.write("  %-8s %-10s %-10s %s\n" % ("width", "phase 0", "worst phase", "symbols scored"))

    for width in WIDTHS:
        marks = []
        counts = []
        for phase in range(width):
            ratio, scored = tail_ratio(carve(data, width, phase), 0x51F7)
            if ratio == ratio:
                marks.append(ratio)
