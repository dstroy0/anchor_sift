...
#
#   Usage:  python tools/Salishan/anchor_sift_algorithmic_extraction/symbol_sift.py <stem>
#
...
#
# THE METHOD IS THE SAME ONE boundary_check USES
#
# A break site is a place where the extraction has two tokens and the paper may have had one. The
# candidates are the two tokens joined by each mark the paper writes, and joined by nothing,
# the reading where the space is a real word boundary.
#
...
# rate at half a percent, the same budget boundary_check spends.
DRAWS = 200
...
# below where a flat distribution would put them, the reading that this paper does not write
...
def flattened(counts):
    ...
    from this reference and never its raw frequency. A run that is common everywhere carries no
    weight, and a run that belongs to one restoration carries it.
    """
...
# decidable anyway, wherever the paper writes the joined form somewhere it was not collapsed, the
# same evidence every other candidate here rests on. Robertson's page 30 names its own symbols as
...
def candidates_for(first, second, inventory):
    ...
    # is a third reading and it is what recovers a labialization.
...
def break_sites(lines, marks, vocabulary, inventory):
    ...
    xʷəlp-í<p>l̓əx stands in section 2.2.2 and xʷəlp-í<p>l əx in example (23). A site is therefore a pair
...
def main():
    ...
    # Every token the paper writes, and a restoration has to be found in one.
