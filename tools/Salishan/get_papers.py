...
#
#   python tools/Salishan/get_papers.py             the papers the readers need
#   python tools/Salishan/get_papers.py --all       every paper in the archive
...
def converted(source, target):
    ...
    if unmapped:
        with open(notice, "w", encoding="utf-8", newline="\n") as handle:
            ...
            handle.write("  python tools/Salishan/pdf2png.py %s 1 <last>\n\n"
...
def report_unfaithful(out, stems):
    ...
    for stem in stems:
        out.write("    python tools/Salishan/pdf2png.py %s 1 <last>\n" % stem)
