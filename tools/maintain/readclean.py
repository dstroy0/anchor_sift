...
that then looks wrong IS wrong, instead of being a mismatch the reader was primed not to see.

WHAT BLIND KEEPS, AND WHY IT IS NOT A GENERIC RENAMER. This tree has one shape, and the shape is
grammar and not vocabulary: an entry is a function-pointer member of an `<X>Ns` table, operands
...
Usage:
    python tools/maintain/readclean.py code   PATH [PATH ...]
    python tools/maintain/readclean.py blind  PATH [PATH ...] [--legend FILE] [--keep NAME,...]
    python tools/maintain/readclean.py claims PATH [PATH ...]
...
# matter where it is invoked from, and os.getcwd() is a property of the caller and not of the
...
# `__attribute__((v7))`, which reads as a call on a variable instead of as the attribute that lets
...
class Blinder(object):
    """One naming table for a whole run, which keeps a name crossing from the .h into the .c as itself.

    The categories are the shape's own, not a compiler's. Nothing here parses C: the pass has to be
    CONSISTENT, not correct, because its job is to remove meaning instead of recovering it. A name
    ...
    def whole(self, text):
        """`text` blinded in one pass, which tells a literal and a directive from a name.
        ...
        # An ENTRY is not a variable: it is the member whose type IS the shape's signature. It
...
def show_claims(p, _state):
    """Every comment, with the code line it introduces, which meets a claim with its subject."""
    text = io.open(p, encoding="utf-8", errors="replace").read()
    m = code_mask(text)
...
def main():
    ...
    if mode == "blind":
        ...
        # puts every name back before a judgement has been formed, the thing this pass exists to
        # prevent.
        if legend:
