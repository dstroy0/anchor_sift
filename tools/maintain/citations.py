...
#
#   python tools/maintain/citations.py           what is registered, and what is used and is not
#   python tools/maintain/citations.py --seed    enter the names found in the tree, fields empty
#   python tools/maintain/citations.py --check   fail while a name is used and unregistered
#   python tools/maintain/citations.py --bypass  answer the gate without satisfying it
#
...
FIELDS = ("key", "author", "year", "title", "identifier", "file", "uses", "first_use")

# The owner's columns. --seed writes them empty and no run of this tool ever writes them again.
ENTERED = ("author", "year", "title", "identifier", "file")
...
def write_registry(root, rows, out):
    ...
    with io.open(target, "w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write("# Entered by tools/maintain/citations.py --seed in anchor_sift, which writes\n")
...
def main():
    ...
    if not os.path.isdir(root):
        ...
        out.write("      python tools/maintain/citations.py --check --bypass\n")
    ...
    for key, row in registry.items():
        ...
        row["first_use"] = hits[0] if hits else ""

    unattributed = sorted(one for one, row in registry.items()
                          if not (row["author"] or row["identifier"]))
    unused = sorted(one for one, row in registry.items() if row["uses"] == "0")
    ...
    if seeding:
        ...
        out.write("      python tools/maintain/corpus_manifest.py --write --root %s\n"
                  % root.replace("\\", "/"))
