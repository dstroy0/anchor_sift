...
# vectors`. A run needs no network, and the same bytes are tested every time. Refreshing is a
# deliberate act: run this, read the manifest diff, and say why the numbers moved.
#
#   Usage:  python tools/maintain/vendor_test_vectors.py [--keep-archives]
#
# What lands, and why each is here instead of any other set:
#
...
def take_hmac_section(archive, stamp, manifest):
    """Copy the [L=32] block of HMAC.rsp, the SHA-256 one of five hash sizes."""
    ...
    header = ("# Extracted from HMAC.rsp, the [L=32] section only, by tools/maintain/vendor_test_vectors.py\n"
              "# Source %s\n# Archive sha256 %s\n\n" % (HMAC_URL, stamp))
...
def main():
    ...
    manifest = {
        "note": "Published SHA-256 vectors, vendored. Refresh with tools/maintain/vendor_test_vectors.py.",
