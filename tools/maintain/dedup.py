...
Three renaming strengths, because they answer different questions:
  ...
  --rename all     every identifier goes, types and callees included. Answers "is this the same
                   shape", and it found one idiom written out forty-eight times.
...
reorder instead of a copy, the case a sequence hash silently misses.
  ...
  --json       machine-readable, for a script that wants the groups instead of the report
"""
...
def tokenize(text, first_line=1):
    ...
    The line travels with the token, which lets a hit be reported where it is instead of only as a
    shape. Nothing downstream reads past the second element, and a shape does not see it.
    """
    out = []
...
def statements(tokens):
    ...
    parentheses, and that separates them.
    """
    out, cur, paren = [], [], 0
    for tok in tokens:
        txt = tok[1]
...
def bag_digest(stmts, rename, fold_nums):
    """A hash of the statements as a set instead of a sequence.

    Each statement canonicalizes on its own. A name introduced in one statement does not fix the
    numbering of the next, and the hash is blind to order because of it.
    """
...
def load(p):
    ...
    This is readclean's `code` view, taken as tokens instead of as lines.
    """
