...
Every script sits at that depth, and none of them counts parent directories to locate the repository. They start at their own directory and walk up until they find `src/engine`. The previous layout counted. Moving a file changed its distance from the root and broke its imports. The breakage did not show up until somebody ran the script.
