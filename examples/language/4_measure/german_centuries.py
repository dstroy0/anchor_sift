...
# definition, capitalization and typesetting all moved a great deal across this span. The years should
    ...
    with zipfile.ZipFile(path) as bundle:
        ...
        for name in names:
            ...
            if len(text) < SAME_LENGTH:
                continue
            found.append(
                (int(year.group(1)), os.path.basename(name)[:-4], text[:SAME_LENGTH])
            )
    return found
...
def main():
        ...
        out.write(
            ...
            % (
                ...
                (
                    (
                        "%d to %d"
                        % (min(row[0] for row in held), max(row[0] for row in held))
                    )
                    if held
                    else "none"
                ),
    ...
    for index, (_, century, _, values) in enumerate(rows):
        ...
        for other in centuries:
            ...
            if not kept:
                continue
            distance = float(
                numpy.linalg.norm(values - numpy.mean(numpy.stack(kept), axis=0))
            )
    ...
    out.write(
        "  a text lands in its own century %d of %d, guessing gets %.1f percent\n"
    ...
    ranked = float(
        numpy.corrcoef(
            numpy.argsort(numpy.argsort(gaps)), numpy.argsort(numpy.argsort(apart))
        )[0, 1]
    )
    out.write(
