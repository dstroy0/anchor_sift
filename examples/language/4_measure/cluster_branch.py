...
#
# The family each language belongs to is checked separately and directly, because the proposal was
# a family gate. If families were the groups, the spread inside a family would be small against
...
# report prints a distance matrix, its tree and the cophenetic check in one block. It sits beside
# this file because it is presentation, and nothing in the engine writes to a stream.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from cluster_profiles import report  # noqa: E402
from measure.clustering import standardized  # noqa: E402
...
def main():
    ...
    # The family label is dropped first, leaving the three numbers the gate would decide from
    made = standardized({name: held[name][1:] for name in names}, names)
