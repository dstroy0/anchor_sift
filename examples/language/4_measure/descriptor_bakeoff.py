...
# group. Guessing is stated for each, which lets a score be read without knowing how many groups there are.
...
from measure.web import marginal, web  # noqa: E402
from measure.word_lengths import LONGEST, length_spread, word_lengths  # noqa: E402
from oracle.language.families import every_family  # noqa: E402

CORPORA = os.path.join(ROOT, "build", "corpora")

RANKS = 64

# Words a text needs before a length distribution is worth reading. Lower than the engine's own
# floor because the Japanese texts here are 30000 characters, and a floor of 5000 words would drop
# the one question this bakeoff exists to ask twice.
LEAST_WORDS = 2000
...
DESCRIPTORS = (
    ...
    ("how often each character is used", RANKS, lambda text: marginal(text, RANKS)),
    ("how the word lengths are spread", LONGEST + 1,
     lambda text: length_spread(text, least=LEAST_WORDS)),
    ("word length, mean and spread", 2, lambda text: word_lengths(text, least=LEAST_WORDS)),
