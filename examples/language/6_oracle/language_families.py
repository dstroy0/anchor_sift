...
from measure.clustering import (
    agglomerate,
    as_brackets,
...
def averaged(rows):
    ...
    return {
        name: numpy.mean(numpy.stack(values), axis=0)
...
def main():
    ...
    out.write(
        "  %d texts over %d languages\n\n"
        % (len(loaded), len({row[0] for row in loaded}))
    )
    out.write("  %-34s %-9s %-14s %s\n" % ("reading", "letters", "found", "share"))

    readings = {}
    for label, ranks, prepare in (
        ("every character it actually uses", RANKS, None),
    ...
    if label in readings:
        ...
        out.write(
            "  a low value means the tree is imposing structure the distances do not carry\n"
        )
