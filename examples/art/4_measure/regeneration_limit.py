...
# Catalog: ART-4-003
#
# Measure how much of a set a summary can put back, for Section 4.11 of theory/workbooks/anchor_sift.
#
...
#
# A frequency decomposition splits any set into how much of each frequency and where each sits. The
...
#
# So four reconstructions are compared against the original. One keeps only the exponent, the summary
# this work has been refining, and is the cheapest at a single number. One keeps every amount and
# discards the positions. One keeps every position and discards the amounts. One keeps both, the whole
# set, and costs everything.
#
...
from measure.spectral import exponent_plane  # noqa: E402
...
def main():
    ...
    for label in sorted(WIDTHS):
        ...
        held = as_grid(whole, width)
        if (held is None) or (held.shape[0] < 128):
            continue
        rows = held.shape[0]
        grid = held.astype(numpy.float64)
        slope, _ = exponent_plane(held.reshape(-1), width)
        if slope is None:
            continue
        ...
        # A field built to the same exponent alone, one number of storage
