...
# bits. A shift is only evidence once it leaves that band.
...
def locate(seats, window, clean_bits):
    ...
    Collision entropy is permutation invariant over whatever span it is computed on. A single figure
...
def main():
    ...
    for rate in (0.0, 0.0002, 0.001, 0.005, 0.02):
        ...
        # Where the block was written, which lets a claim of having found it be checked
        count = int(len(seats) * rate)
        placed = -1
