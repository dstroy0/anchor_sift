...
def octet_closes(molecule):
    ...
    return all(degree[node] == VALENCE[molecule.atoms[node]]
...
def main():
    ...
    for molecule in CATALOG:
        ...
        out.write("  %-20s %-9s %-7d %-7d %s\n"
                  % (molecule.name, formula(molecule), len(molecule.atoms),
                     len(molecule.bonds), "closes" if held else "OPEN, check the bonds"))

    out.write("\n  positive control: %d of %d built molecules close every atom.\n"
              % (closed, len(CATALOG)))

    isomers = {spelled: names for spelled, names in by_formula.items() if len(names) > 1}
    out.write("  a formula does not fix a molecule. %d formula(s) hold more than one structure:\n"
              % len(isomers))
    ...
    out.write("\n  %s\n" % ("every built molecule closed." if ok
