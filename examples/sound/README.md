...
| `4_measure` | `coherent_hum_removed_to_the_bit.py`, `impulses_removed_to_the_bit.py`, `self_similar_removed_to_the_bit.py`, `outliers_removed_without_a_threshold.py` |
...
A **coherent** noise repeats, so the invariant is POSITION. A hum adds the same short cycle over and over, its energy gathers into the phase classes of its period, and `measure/periodic_energy` reads that period off how far the phase grouping stands above a shuffle. `reference/periodic` builds the phase mean and the residual is the target with the hum gone. `coherent_hum_removed_to_the_bit.py` removes it to the last bit where the target sits orthogonal to the hum's period. This is a comb filter.

An **incoherent impulse** does not repeat, so it is identified by the target it lands on. When the target repeats, `measure/shift_agreement` reads its period and `reference/periodic` restores each corrupted sample from the value its phase class agrees on. `impulses_removed_to_the_bit.py` removes them to the last bit where each corrupted class keeps a clean majority.

A **recurring motif** is identified by CONTENT rather than position. `reference/self_similar` groups the places that share a surrounding context and averages their centers, so a motif occurring at positions with no period between them is still recovered. `self_similar_removed_to_the_bit.py` is non-local means reached through the same construction, and it reads what a comb filter cannot, because it never asked for a period.

An **outlier on a locally flat signal** is identified by NEARNESS. `measure/local_outlier` flags a sample that leaves the band its own neighbours draw, and `reference/windowed` replaces it with their median. `outliers_removed_without_a_threshold.py` is the Hampel filter with its magic number removed: the band is drawn from the neighbourhood instead of chosen as a multiple of the median absolute deviation.

`period_background_and_its_null.py` is the stage three reading under the first two: the phase-mean background at the true period against the same background on a shuffle, so the part of the reading that clears the null is visible.

### What these are and are not

Each script runs on a **synthetic positive control built in the script**, because a full rejection can only be proved against a signal whose clean form is known. Every one reports a **drawn null** and a **stated floor**: the noise shaped exactly like the target, which no instrument can reject. Every one runs **two independent routes** that agree bit-exact where both hold and split where the data no longer carries an answer, and each shows that the routes can genuinely disagree, so their agreeing is evidence and not an identity typed twice.
...
- **The matched filter is already here** as the anchor sift: a subset of a pattern's points is a necessary condition over any index set, which is the necessary-condition half of matched filtering, in `src/engine/python/sift/`.
- **Excluded by principle: the model-based filters.** Wiener needs the signal and noise power spectra, and Kalman and the adaptive LMS and RLS filters learn a model from the stream. This engine has no model, no prior and no training by construction, so these are outside it not by capability but by what it refuses to assume.
- **Excluded by the pure-Python constraint: the transform filters.** A low-pass, high-pass or band-pass filter, wavelet thresholding and spectral subtraction all read a frequency basis, which is the dense-array and FFT path the dependency ban keeps out. The engine's own answer to scale is `partition/coarsen`, reading at the scale where structure sits rather than filtering a spectrum. A true spectral filter belongs to a numpy port, stated as such.
- **Excluded by no bounding: the tuned-tolerance filters.** Bilateral filtering picks an intensity kernel width, total-variation denoising picks a regularization weight, and the classic Hampel picks a multiple of the median absolute deviation. Each is a judgement-picked tolerance. Where the tree has a relative it is the one that draws the boundary instead: non-local means with an exact context in place of a bilateral bandwidth, and the drawn-band outlier filter in place of the MAD multiple.
