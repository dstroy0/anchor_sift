...
- **Excluded by the pure-Python constraint: the transform filters.** A low-pass, high-pass or band-pass filter, wavelet thresholding and spectral subtraction all read a frequency basis, which is the dense-array and FFT path the dependency ban keeps out. The engine's own answer to scale is `partition/coarsen`, reading at the scale where structure sits. A true spectral filter belongs to a numpy port, stated as such.
