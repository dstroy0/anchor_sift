...
\texttt{examples/\allowbreak{}games/\allowbreak{}\{1\_\allowbreak{}represent,3\_\allowbreak{}reference,4\_\allowbreak{}measure\}/\allowbreak{}}
...
A square is a point and the piece on it is its value. Reversi on sixty four squares, two colours.
...
a uniformly drawn order. It keeps how many of each colour are on the board and which squares are
occupied at all, and destroys only which colour sits where. Everything a rule imposed is in what it
destroys.

The measure is the share of occupied squares whose neighbour one step away carries the same colour,
averaged over both axes. The dispersion measure this work usually quotes needs four symbols clearing
an occurrence floor and a board has two colours, so this reads the same thing at the one lag a board
...
1.2222 and 1.2282, a spread of 0.0089, so the departure is about twenty five times the spread between
batches.

A departure on its own does not say which ingredient produced it, and a board supplies the control
for nothing. Play the same board with the same two colours and the same number of squares filled,
...
The first version of that control kept Reversi's four fixed centre pieces and read 0.971, three
floors below the null where the argument wanted one. The gap is the opening itself. Those four
squares are laid out black over white and white over black, so all four neighbouring pairs inside
them disagree by construction where a draw would have half of them agree. Four guaranteed
disagreements out of a hundred and twelve pairs is 0.018 of the reading, which is the whole of the
gap, and starting from an empty board returns 0.998.

It is kept because it points the other way as well. The same four squares sit under every Reversi row
above and push them in the same direction, so the 1.225 is understated by about that much and is
never overstated.
...
move turns over a bracketed run, whoever chose it, so a strong player and a weak one leave the same
kind of trace and differ only in where on the board they put it. The measure reads what was done to
...
which is the correct behaviour: it is asked for a period and reports one with no confidence at all.
