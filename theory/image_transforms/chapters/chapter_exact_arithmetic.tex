...
correlation agrees to the digit with the direct $O(N^2)$ count and recovers the planted shift, so its
...
frames, pooling neighbours, smoothing a field. Where the arithmetic makes no error there is nothing to
...
the grid spacing, so it is the integer counterpart of a Gaussian of that variance. Separable along each
...
scale with nothing divided. Its width is fixed by the orders and the input depth, so it is carried in a
...
$p = 119 \cdot 2^{23} + 1 = 998244353$, with $p - 1 = 2^{23} \cdot 7 \cdot 17$, so 3 is a primitive
...
\texttt{theory\_bucket/\allowbreak{}twiddle\_constants\_article.tex}. This prime is below $2^{31}$,
...
Exactness has a stated bound. Each coefficient is a sum of products $a(v)\, b(v + \ell)$, so for binary
...
alias, and the transform is separable, one axis at a time, so the construction holds in any number of
dimensions.
...
no rounding in it, so the improvement from adding a stage will not shrink the way it does in a floating
