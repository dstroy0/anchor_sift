\chapter{The method of precision measurement}

\section{Leverage pattern occurrence}

\textbf{Purpose:} Determine what identity gains in information versus what it takes to compute. \textbf{Test scope:} the four benches under \texttt{bench/\allowbreak{}driver/\allowbreak{}} at anchor\_sift commit \texttt{d65d219}, the digest test support of Appendix A, and the engine under \texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}engine/\allowbreak{}} at commit \texttt{1044ca6} (Section 1)

A contrapositive is shown to dwell at test position. One read of one cell settles every alignment whose pattern symbol differs from what was found, which is \(m(1-2^{-H_2})\) of them in expectation, confirmed to within 0.7\% over 84 rows spanning three selection measures. The collision probability that sets the candidate rate therefore also sets the contrapositive indication distance, appearing once as a rate and once as a length.

The identity candidate count is shown to be an estimator of Renyi entropy of order two, equal in expectation to the plug in estimator when the probe's own match is counted and to the standard unbiased estimator when it is not. Candidate hits are scored against published estimators on sources with known parameters. When the term is scored that way it is worse in both accuracy and running time at every setting tested. It converged toward the unbiased estimator as the probe budget approached the corpus length.

Its only advantage is the constraint it survives: it builds no histogram and never enumerates the alphabet. The correction is the exclusion of a coincident index, and recursively applying gives the unbiased estimators at higher Renyi orders. Its magnitude at order two has the closed form \((1-C)/((N-1)C\ln 2)\) bits for a plug in collision probability \(C\), confirmed across nine sources spanning four orders of magnitude. The numerator vanishes exactly at a point mass. On a source with no entropy in it the correction is identically zero at every order and every length.

Put against the classical algorithms on one corpus and one counter, anchoring at the rarest symbol loses to a Horspool shift on every corpus and needle length by 1.2 to 25 times, because an anchor at offset \(a\) can advance by at most \(a+1\), and advance governs the number of symbol accesses. Candidate rate does not. Treating the two as factors of one product recovers the loss and comes out 11\% ahead on C source. Replacing the frequency model with a running advance per offset, which needs no alphabet, comes out 13.2\% ahead and beats a rule handed the corpus histogram outright.
...
A search for a pattern in a body of data can test every condition, or it can test the ones we know
are more probable. The second idea is what seems immediately attractive as a performance option,
test a preknown win condition. Delta null tests every possible position of every constituent of the
object under examination using no information other than the object's identity.

Two questions about that arrangement get answered together in most treatments and come apart under
examination. Whether delta null is safe to use is a question about its method. How much delta null
saves is an answer the object under examination defines using its identity. This theory separates
them for experimentation, examination, and verification.

By section: 2 covers method, 3 describes how to measure, 4 reports on what was measured, 5
discusses results, 6 states any limits hit, 7 and 8 cover any prior works or art and any open
items.

Many different applications and experimental apparatus are available for use in anchor\_sift, the
repository this body of work came from. See \texttt{examples/\allowbreak{}} for application and
\texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}} for the engine and its benches
(\texttt{docs/\allowbreak{}setup.md:29-\allowbreak{}40}).

Paths into anchor\_sift are cited at commit \texttt{1044ca6} unless a line names another commit.
\texttt{anchor\_sift.h:N} and \texttt{anchor\_sift.c:N} are lines of
\texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}engine/\allowbreak{}anchor\_sift.h} and
\texttt{.c}, and paths under \texttt{docs/\allowbreak{}}, \texttt{maint/\allowbreak{}} and
\texttt{theory/\allowbreak{}workbook/\allowbreak{}} are in the same tree. The bench rows in this
chapter were taken on four benches,
\texttt{bench\_\allowbreak{}ancorae\_\allowbreak{}\{sift,\allowbreak{}lattice,\allowbreak{}entropy,\allowbreak{}ab\}.\allowbreak{}c}.
A line cited in one of them is a line of \texttt{bench/\allowbreak{}driver/\allowbreak{}} at commit
\texttt{d65d219}, the parent of commit \texttt{54aa8c3}, which moved the four to
\texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}bench/\allowbreak{}} as
\texttt{bench\_\allowbreak{}\{sift,\allowbreak{}lattice,\allowbreak{}entropy,\allowbreak{}ab\}.\allowbreak{}c}.
This chapter records no run of the moved files.
...
\subsubsection{2.0 Definition}

\begin{center}
  \begin{tabular}{@{}p{2.2em}p{7.5em}p{0.56\textwidth}@{}}
    \toprule
    \(\Sigma\)   & the universe    &
      The underlying set of distinct states or symbols. Lacking intrinsic topology, metric, or
      order, it provides only the bare primitive of distinctness: \(x_1 = x_2\) or \(x_1 \neq x_2\). \\
    \addlinespace[0.7em]
    \(\delta_0\) & the measurement &
      The indicator function of equality. It collapses ambient positional uncertainty into a
      binary bit of information, match or non-match, at a group displacement \(d \in G\). \\
    \addlinespace[0.7em]
    \(\partial\) & the boundary    &
      The anchor set \(A \subset D\), acting as the boundary between candidate space and rejected
      space. \\
    \bottomrule
  \end{tabular}
\end{center}

Checking \(\mathrm{Anc}_A(s)\) determines whether a shift state \(s \in G\) resides inside the feasible
region or crosses the boundary into the rejected exterior. This is survivorship in the algorithm,
and it loses nothing: every true occurrence survives, and the exact compare removes the rest
(\texttt{docs/\allowbreak{}steering.md:10}, \texttt{:118}). The contrapositive \(\neg\,\mathrm{Anc}_A(s) \Longrightarrow \neg\,\mathrm{Occ}(s)\) means
that hitting the boundary once permanently excludes the point \(s\) from the search space.

While the selection of \(A \subseteq D\) is free with respect to correctness, the boundary geometry
\(\partial A\) governs search efficiency. The density and entropy of \(\Sigma\) over \(A\) determine the
flux of rejected candidates passing through the filter per operation. That holds where anchors are
independent, and Section 4.6 measures them failing to be. The engine steers by survivor counts,
which carry the correlation between positions that a histogram leaves out
(\texttt{docs/\allowbreak{}steering.md:42-\allowbreak{}44}).

Because the boundary is defined strictly via displacement operations in \(G\) and \(\Sigma\), the
engine remains invariant under translation, permutation, or change of coordinate representation.
The count is invariant by Proposition 1, and Section 4.2 measures it holding across geometries. Of
the cost, only a rotation is measured (Section 4.2), and invariance of the cost under the other
transforms is a conjecture.

\subsubsection{2.1 Ambient Algebra and Pattern Support}

Let \(\Sigma\) be the carrier of symbols. No structure on \(\Sigma\) is assumed: no order, no metric, no finite cardinality, and no way to enumerate its members. The only operation available is a decidable equality between two positions. The engine asks the same, and two things more: that the equality be a pure function of the two positions for the call, and that the caller supply the number of alignments, which the engine cannot compute (\texttt{anchor\_sift.h:354-357}, \texttt{:374-375}, \texttt{:384-387}).

Let \((G,+)\) be an additive group of displacements and let a domain be a map \(x\) from an admissible subset of \(G\) to \(\Sigma\). A pattern is a finite displacement configuration with support \(D \subset G\), together with the symbols found at those displacements from a chosen base position. For a base \(t\) and a candidate base \(s\), define
\[
...
\]
is not entailed, in any domain and in any state. A witness is any \(s\) agreeing with the base on \(A\) and differing on some \(d \in D \setminus A\), and the definitions permit such an \(s\) whenever \(|\Sigma| > 1\). The converse is true for one \(A\) only, \(A = D\), where the filter has become the verification.
...
\noindent\textbf{The selection rule is free.} Proposition 1 quantifies over every subconfiguration and never mentions what chose one. No frequency table, no random table, and no absence of a table can produce a false negative from a probe over the symbols themselves. A probe over rarity ranks carries one further condition, stated below. What a measure moves is how often the rejection fires.

\noindent\textbf{The alphabet need not be known.} Both predicates are equalities between positions. Where \(\Sigma\) cannot be enumerated no table indexed by symbol exists, and rarity can still be counted through equality alone, at a cost of up to the square of the field length in oracle calls (\texttt{anchor\_sift.h:403}, \texttt{:497-500}). The flat measure is what remains for a rule that counts nothing, and maximum entropy is the base case for that rule. A weighted table is the special case available on a known domain.

\noindent\textbf{The displacement support need not be ordered.} The definitions use \(+\) and never \(<\). A grid, a volume, and a rotated point configuration differ only in which members of \(G\) appear in \(D\). A rotation is a permutation of \(D\), which changes no term in either predicate.

\noindent\textbf{A rarity projection is free only inside one population.} A rank probe is guaranteed to be a necessary condition only where pattern and domain were ranked together. The class a position falls in comes from the caller's relation and carries to another field when that relation is transitive. The rarity order is counted over one population and holds only inside it, so ranks from two populations can refute a true occurrence (\texttt{anchor\_sift.h:408-413}, \texttt{:419-429}; Section 2.7).

\subsubsection{2.5 Probabilistic Model of Symbol Accesses}
...
\]
\(\mathbb{E}[C_1]\) is what the verify step reaches and \(\mathbb{E}[S]\) is the run a skipping search never touches. Reducing \(N\) candidates to \(O(1)\) survivors requires \(\log_2 N\) bits of discrimination and one anchor supplies \(-\log_2 q\) bits.

The second expression is Kac's lemma. For a stationary ergodic source the expected return time to a set of measure \(q\) is exactly \(1/q\), which makes the skip a recurrence time and not an artifact of the filter. That names an assumption the construction does not supply. Proposition 1 guarantees that no occurrence is lost, and Section 2.5 counts the symbol accesses, and neither says a pattern occurs at all. That last guarantee belongs to the source: a stationary ergodic process visits any event of positive probability almost surely, and there is no such promise for a process that is not stationary or not ergodic. Three guarantees with three owners, and only the first is this document's.

For an uninformed anchor, the symbol is drawn from the domain by the domain's own frequencies,
...
\begin{tikzpicture}[
  ...
  \node[test, right=of pos]           (anc)  {anchor \(a \in A\)\\[-2pt]
                                              \(x(s+a) = x(t+a)\)?};
  \node[box, right=of anc]            (cand) {candidate\\[-2pt]
                                              proves nothing};
  \node[final, below=of anc]          (ref)  {refuted\\[-2pt]
                                              exact, final, never revisited};
  \node[verify, below=of cand]        (ver)  {exact compare\\[-2pt]
                                              every \(d \in D\)};
  \node[box, below left=11mm and 1mm of ver]  (occ) {occurrence};
  \node[box, below right=11mm and 1mm of ver] (fp)  {false positive};

  \draw[edge] (pos)  -- (anc);
  \draw[edge] (anc)  -- node[lab, above] {passes} (cand);
  \draw[edge] (anc)  -- node[lab, left]  {fails}  (ref);
  \draw[edge] (cand) -- (ver);
...
\noindent\textbf{Figure 1.} The anchor filter, as \texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c:193-\allowbreak{}239} implements it. Refuted is the only box an anchor reaches with certainty. The exact compare reaches the other two, and everything below the candidate is work.

\subsubsection{2.7 The Implementation and Its Guarantees}

The propositions above need no code. This section states what the engine at anchor\_sift commit \texttt{1044ca6} guarantees, the line that carries each guarantee and the condition each one needs, and it records the readings of the engine that were withdrawn.

\noindent\textbf{The count is exact for any legal probe set on a byte field.} Each probe compares the corpus with the pattern's own symbol at an offset inside the pattern (\texttt{anchor\_sift.c:1337}). Each probe is therefore a necessary condition, a probe set is their conjunction, and every alignment the probes leave standing reaches a full compare (\texttt{:1349-1354}). Legal means every position the probe reads lies inside the pattern (\texttt{anchor\_sift.h:844-845}). \texttt{anchor\_steer\_count\_with\_probes} takes the caller's probes without applying that test (\texttt{anchor\_sift.c:1305-1358}). An illegal probe passed to it reads past the pattern, and nothing refuses it. A refusal returns 0, the value a count of zero also returns (\texttt{:1309-1312}, \texttt{:1321-1324}). An empty pattern occurs at every alignment, and \texttt{anchor\_sift\_naive}, \texttt{anchor\_steer\_count} and \texttt{anchor\_steer\_count\_with\_probes} each return the corpus length plus one for it (\texttt{:94}, \texttt{:1372-1375}, \texttt{:1317-1320}). On a rank field the full compare compares ranks, so the count there is exact only where one rank names one class (below).

\noindent\textbf{Nothing accumulates, and the engine is not a streaming algorithm.} Every comparison is an exact equality. A descent sets every alignment standing once, when it starts, and after that only clears them (\texttt{anchor\_sift.c:948-953}, \texttt{:621}, \texttt{:638}), so every level it reaches is a complete probe set and yields the same count as any other (\texttt{docs/\allowbreak{}steering.md:83}). The planning can therefore stop at any level. The sweep over alignments cannot, because stopping it early loses occurrences (\texttt{:85}). A descent places probes and counts nothing. Its output is the survivor vector, one byte per alignment (\texttt{anchor\_sift.h:666-670}), which grows with the field (\texttt{docs/\allowbreak{}steering.md:69}). Space bounds proved for streaming matchers describe a different model (Section 7.5), and the reading of the engine as sitting at a space lower bound is withdrawn (\texttt{docs/\allowbreak{}findings-\allowbreak{}for-\allowbreak{}verification.md:294-\allowbreak{}310}).

\noindent\textbf{Placement is greedy coverage.} Write \(R_p\) for the alignments a probe \(p\) rejects. Then \(f(P) = |\bigcup_{p \in P} R_p|\) is monotone and submodular (\texttt{docs/\allowbreak{}steering.md:210-\allowbreak{}214}). \texttt{anchor\_\allowbreak{}steer\_\allowbreak{}spawn\_\allowbreak{}coarms} keeps the candidate that leaves the fewest alignments standing (\texttt{anchor\_sift.c:991-1000}). Fewest standing is the largest marginal gain in \(f\), and by Nemhauser, Wolsey and Fisher (1978) the coarms it places therefore reject at least \(1 - 1/e\) of what the best set of as many single position probes rejects (\texttt{anchor\_sift.h:783-790}). The bound counts alignments rejected, and two probe sets rejecting the same alignments can cost different numbers of reads. It holds only at a \texttt{sample\_stride} of one, where every gain is scored exactly (\texttt{:792-796}). \texttt{anchor\_steer\_plan\_recursive} chooses among the caller's own offsets and may place all of them (\texttt{anchor\_sift.c:968}), so the best set of that size is the whole set, and the ratio holds for it trivially and guarantees nothing. Stopping at zero gain is equivalent to continuing only where the candidate set never grows between levels, and submodularity does not supply that (\texttt{docs/\allowbreak{}steering.md:220-\allowbreak{}224}).

\noindent\textbf{A descent stops, recurses, refuses or runs out of candidates, and it never revisits a state.} A refusal returns 0 and writes nothing (\texttt{anchor\_sift.c:918-944}). An offset already placed is never scored again (\texttt{:977-989}). A descent with no unplaced candidate left stops without the destroy rule (\texttt{:1003-1006}): the pattern \texttt{ab} spawned into the corpus \texttt{abxbay} with four coarms asked for places two, with \texttt{force\_full\_depth} off and on alike. The return is at most the count asked for (\texttt{anchor\_sift.h:707-710}, \texttt{:767-769}). Depth is at most \texttt{ANCHOR\_STEER\_ANCHORS}, defined as \texttt{ANCHOR\_SIFT\_ANCHORS}, which is 4 (\texttt{:90}, \texttt{:39}). Below that bound the count asked for, the candidates available and the corpus decide how deep a descent goes. The destroy test compares survivor counts read off the corpus, and \texttt{force\_full\_depth}, which overrides it, is zero unless the caller sets it (\texttt{anchor\_sift.c:1019-1023}; \texttt{anchor\_sift.h:631-634}, \texttt{:738-743}). The reading that depth is fixed before the program runs is withdrawn. The bench cascades of Section 4, which reach six anchors, are a different program.

\noindent\textbf{Where the reads run.} The counting entries are written as scalar loops (\texttt{anchor\_sift.c:94}, \texttt{:1327}, \texttt{:1389}), and so is a descent over a field of any symbol type (\texttt{:511-531}), because an equality oracle hides the representation a wide compare reads. The only wide path is the planner's survivor scan on the byte path at a stride of one (\texttt{:537-562}). It takes the widest arm the build carries and the processor reports, AVX-512 before AVX2 on x86 and SVE before NEON on ARM, with the portable arm last (\texttt{:1423-1466}). The scan arms and the exact integer arms each keep one file per instruction set, for the same six sets: portable, AVX2, AVX-512, NEON, SVE and CUDA (\texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}engine/\allowbreak{}scan\_*}, \texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}no\_rounding/\allowbreak{}arm\_*}). A check fails when the two lists differ (\texttt{maint/\allowbreak{}engine/\allowbreak{}check\_arm\_families.py:4-14}). Both families compute one operation, an equality at a fixed displacement counted over positions. A scan arm counts the standing alignments whose byte at one offset is the wanted byte (\texttt{docs/\allowbreak{}arm-\allowbreak{}verification.md:6}), and an exact arm counts the positions whose value equals the value one lag away (\texttt{src/\allowbreak{}engine/\allowbreak{}c/\allowbreak{}no\_rounding/\allowbreak{}exact\_integer.h:228-\allowbreak{}235}). The AVX2, NEON and CUDA scan arms have run against the portable arm with no disagreement over the grid in \texttt{docs/\allowbreak{}arm-\allowbreak{}verification.md:18-\allowbreak{}25}. The AVX-512 and SVE scan arms have never run and are graded only on the instructions their compiled objects contain (\texttt{:29-38}). The CUDA arm is outside the dispatch (\texttt{anchor\_sift.h:1160-1162}).

\noindent\textbf{Any symbol type, through equality, and ranks inside one population only.} The engine's interface to a symbol is \texttt{AnchorSameAt}, equality at two positions (\texttt{anchor\_sift.h:354-357}, \texttt{:377}), and a descent reaches a field of any symbol type through that alone (\texttt{anchor\_sift.c:526}, \texttt{:619}). \texttt{anchor\_field\_project} turns such a field into byte ranks, rarest first (\texttt{anchor\_sift.h:403}, \texttt{:485-487}). A rank fuses a class, taken from the caller's relation, with a rarity place counted over the population the call saw, and the place does not carry to another field (\texttt{:408-413}), so ranks from two separate calls can refute a true occurrence (\texttt{:419-429}). Classes are the transitive closure of the relation over the positions present (\texttt{:443-453}; \texttt{anchor\_sift.c:684-706}), at a cost of up to the square of the field length in oracle calls (\texttt{anchor\_sift.h:497-500}). \texttt{anchor\_field\_pair\_project} numbers corpus and pattern as one population (\texttt{:586}; \texttt{anchor\_sift.c:885-888}), so one class takes one rank on both sides and no true occurrence is lost under any oracle (\texttt{anchor\_sift.h:450-451}, \texttt{:604-605}). Its rank count is exact at 256 classes or fewer under a transitive oracle (\texttt{:596-598}). Past 256 classes, or under an oracle whose closure joins positions that do not agree, the count is an upper bound (\texttt{:450-453}, \texttt{:600-603}).

\noindent\textbf{Unbuilt and open.} A reader the engine could call for more field is described in prose, with no signature or contract, and nothing is being built toward it (\texttt{docs/\allowbreak{}findings-\allowbreak{}for-\allowbreak{}verification.md:241-\allowbreak{}257}). Whether a caller's loop over descents is universal is open. The target named for it is a tag system (Minsky, 1961), no encoding into that target is written, and the engine as built is finite (\texttt{docs/\allowbreak{}steering.md:160}, \texttt{:188-\allowbreak{}192}).

\subsection{3. Experimental Design}

\subsubsection{3.1 What runs the measurements}

Two kinds of program, answering two kinds of question. The benches answer what the propositions
claim, on data written to have a known answer. The examples answer what a domain does, on data
nobody here made.
...
\midrule
\texttt{bench\_ancorae\_sift.c} &
  admission and independence over byte strings &
  candidates, skip, \(I(d)\), \(z\), refusals \\
\addlinespace[0.4em]
\texttt{bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c} &
  whether Proposition 1 survives off the line &
  candidates, refusals \\
\addlinespace[0.4em]
\texttt{bench\_\allowbreak{}ancorae\_\allowbreak{}entropy.\allowbreak{}c} &
  what the candidate count estimator is worth &
  bias, root mean square error \\
\addlinespace[0.4em]
\texttt{bench\_ancorae\_ab.c} &
  how a whole search compares to one built another way &
  corpus symbol accesses \\
\bottomrule
\end{tabular}
\end{center}

\noindent\textbf{The benches are not the engine.} They cascade up to six anchors, and the descent of Section 2.7 places at most four (\texttt{anchor\_sift.h:39}, \texttt{:90}).

The examples under \texttt{examples/\allowbreak{}} are 122 scripts over nine domains, each one a stage of the same pipeline, and every domain result in the other theories comes from one of them. Appendix B lists the stages and what each domain reaches.

Nothing anywhere is timed. No row is a performance claim, and every number is a property of the
data and the geometry, identical on every part.

\subsubsection{3.2 What every row has to report}

Every row of every bench reports two things, and they are graded differently.

\noindent\textbf{Refusals.} The count of true occurrences an anchor rejected. Proposition 1 says this has one
acceptable value, and it is zero. A nonzero entry is a defect in the bench and never a property of
a domain: no distribution, no geometry and no alphabet can reach a claim that quantifies over every
subconfiguration. In the engine an impure oracle (\texttt{anchor\_sift.h:374-375}) and ranks from two separate projections (Section 2.7) are such defects. A row that reports a refusal records a defect to fix and is never read as a finding.

\noindent\textbf{Admissions.} The count of positions that passed the anchors and reached the exact compare.
This one is expected to move, with the measure, the anchor count and the geometry, and Section 4
is the record of it moving. It has no correct value.

A case with nothing to check prints \texttt{none} instead of zero. A row that had no work to do
cannot be read as a row that did the work and found nothing
(\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:715-\allowbreak{}757}).
...
Needles are drawn from the corpus so every one genuinely occurs. A corpus built by repeating a unit is self similar at that scale and every needle then occurs once per repetition by construction, which ruined an earlier measurement recorded in Section 6.5. The fill truncates instead of repeating (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:190-\allowbreak{}193}).

\subsubsection{3.4 Anchor-Selection Policies}

Three, differing as much as the interface allows (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:342-\allowbreak{}347}, \texttt{361-\allowbreak{}420}):
...
\texttt{maxent} & one cost for every byte. The table carries no information \\
\bottomrule
\end{longtable}
\normalsize
...
Six cases share one core that receives a list of valid base positions, a list of displacements, and a callback answering whether two positions carry the same symbol (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c:80}, \texttt{193-\allowbreak{}239}). The core has no dimension parameter, because the geometry is entirely inside the base list, and no symbol type, because it only asks whether two agree.
...
The unbiased estimator and the first order bias correction of the plug in estimator are the same expression, which makes them one estimator and not two (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}entropy.\allowbreak{}c:256-\allowbreak{}344}).

A fifth quantity, the most common value estimate of min entropy from NIST SP 800-90B section 6.3.1, is computed for context and is not scored on the same scale, because min entropy is a different property (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}entropy.\allowbreak{}c:379}).
...
\noindent\textbf{It stops in three places, and every failure recorded in this document is one of them.}

\noindent\emph{The number is not unique, it is indexed by a slice.} There is no \(H_2\) of a corpus, only \(H_2(w)\) of a corpus read \(w\) bits at a time, and Section 4.10 measures it falling from 0.984 to 0.600 bits per bit between \(w = 1\) and \(w = 16\) on English. Every figure above uses \(w = 8\), which nothing here justifies.

\noindent\emph{A second moment is not a distribution.} Section 4.3.1 needs the expected minimum of \(m\) size biased draws, which is a property of the whole order statistics. Entropy orders the corpora wrongly there, and Section 4.8 prices a mismatched reference at 25 to 30 times, which no collision probability predicts.

\noindent\emph{Independence is assumed throughout and fails.} Section 4.6 measures the product rule wrong by four orders of magnitude, Section 4.7 finds correlation surviving to separation seven, Section 4.6.1 has 2.98 anchors predicted against 6 measured at \(m = 16\), and Section 4.9.3 has the distance field losing on periodic data alone. The parameter predicts none of these, because each is a statement about the joint distribution and it is a statement about a marginal.
...
\noindent\textbf{582 rows hold. 9,396,207 true occurrences examined. 0 refused.}
...
\noindent\textbf{252 rows hold. 213,840 true occurrences examined. 0 refused.}
...
\noindent\textbf{A rotation is invisible.} \texttt{grid2d\_\allowbreak{}scatter} and \texttt{grid2d\_\allowbreak{}turned} are the same eight points a quarter turn apart and agree at every anchor count, which puts Section 2.4's third corollary on a measurement.

\noindent\textbf{The complex alphabet behaves as bytes do.} \texttt{field1d\_\allowbreak{}complex} matches \texttt{line1d} to within 0.1\% at every count, with the geometry held fixed and only the symbol type changed.

\noindent\textbf{The cost law holds at every dimension swept.} The hypercube cases place one pattern point at the origin and each of the rest one step out along the next axis. The pattern touches a new axis for every point it has and is never a lower dimensional figure sitting in a larger space (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c:686}). Against \(1 + (P-1)2^{-n}\) for \(P\) positions and \(n\) anchors, which includes the one match the pattern is guaranteed against itself:
...
Across all 48 rows the ratio stays between 0.969 and 1.028. Seven points off the origin can touch at most seven axes. The dimension eight pattern spans seven of its eight, which is a limit of the pattern size and not of the construction.
...
\noindent\textbf{The ceiling is exactly 1 if and only if \(p\) is uniform on its support.} If every nonzero \(p_\sigma\) equals \(1/k\) then \(F\) is a single step, the integral collapses to \(1/k\), and \(\sum_\sigma p_\sigma^2\) is also \(1/k\). If two nonzero probabilities differ then the rarest of \(m\) draws is strictly below the size biased mean for \(m \ge 2\) and the quotient exceeds one. A point mass and a uniform distribution are both uniform on their support. Both ends of Section 4.3 report a ratio of one, and entropy plays no part in either case.

Computed at needle 16 from each corpus's own frequencies (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:933}):
...
The quantity that sets the candidate rate in Section 4.4 sets the contrapositive indication distance here, appearing once as a rate and once as a length. Measured at every needle length over every corpus (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:1024}):
...
\noindent\textbf{The probe counting its own match reproduces the plug in estimator, and excluding it reproduces the unbiased estimator.} At \texttt{uniform256} and \(N=256\) the plug in bias is \(-0.984\) bits and the probe with self is \(-0.964\); the unbiased bias is \(+0.029\) and the probe without self is \(+0.074\). The subtraction that Section 6.5 records as a measurement error is the standard bias correction, reached from the other direction.

\noindent\textbf{The probe is worse than both published estimators at every setting tested.} Its error is set by the probe budget and not by the corpus length. Root mean square error at \(N = 16384\), \texttt{zipf1.\allowbreak{}0}:
...
The correction excludes a position from being paired with itself. Applied \(k\) deep it excludes every tuple in which any two indices coincide, which is a falling factorial in the numerator and the denominator, giving the unbiased estimator of \(\sum_\sigma p(\sigma)^k\) (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}entropy.\allowbreak{}c:463-\allowbreak{}500}). So the correction can be climbed, and each rung estimates Renyi entropy at the next order.

At order two the correction has a closed form. Writing \(C = \sum_\sigma \hat p(\sigma)^2\) for the plug in collision probability, the corrected form is \((CN-1)/(N-1)\). The two differ by
...
That numerator answers the question the ladder was built for. A point mass has \(C = 1\) exactly. \(1 - C\) is exactly zero and the correction is exactly zero, for every length. The same holds at every order: with one symbol \(n_\sigma = N\), the plug in form gives \(\sum \hat p^m = 1\) and the corrected form gives \(N^{(m)}/N^{(m)} = 1\). Measured at orders 2 through 5 and lengths 256, 1024, 4096 and 16384, every entry is 0.0000 bits.
...
Ratio of observed candidates to the independence prediction, needle 16 (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:541-\allowbreak{}632}):
...
\noindent\textbf{The gap between predicted and measured exhaustion is the independence failure of Section 4.6, read a second way.} \texttt{structured} has the largest cascade ratio and the largest gap. \texttt{flat} carries no bits at all. No stack of any size removes anything, and the row that never exhausts is the same corpus every other instrument reports zero on.

A bench stack exhausts the space at three anchors on \texttt{uniform}, at three or four on \texttt{periodic16} at \(m = 64\) and 256 and four at \(m = 16\), and at three on \texttt{english} at \(m = 64\) and 256 and six at \(m = 16\). \texttt{structured} has not exhausted by six and \texttt{flat} never does. Where a stack exhausts, it exhausts because the space is spent. The engine's descent is a different program, capped at four levels, and the count asked for, the candidates available and the corpus decide how deep it goes (Section 2.7).
...
\(I(d)\) is an exponentiated pointwise mutual information. \(\mathrm{PMI}(d) = \log_2 I(d)\) and the second anchor's contribution is \(-\log_2 p_\beta - \mathrm{PMI}(d)\). At \(I = 5.82\) the loss is 2.54 bits. An anchor nominally worth 8 delivers 5.5.

For fixed width records of period \(T\) with field offsets \(F\), a layout bound predicts

\(D_x \;\subseteq\; \{T\}\ \cup\ \{\,|f_i-f_j| \;:\; f_i,f_j\in F\,\}\)

Here \(T = 16\) and the punctuation sits at \(F = \{4,11,15\}\). \(\hat{D} = \{4,7,11,16\}\). Correlated is \(|z| > 2\), and lags 1 and 2 are excluded because short range correlation is present in every corpus here, English included at \(I(1) = 6.05\).
...
The prediction that a corpus matched table wins is refuted. \texttt{generic} beats \texttt{english} on English prose, and by more on C source. So the case for instantiating the table is not that it should match the data, which is unsupported. The case is that the wrong table costs 25 times and a build holds one.

On the uniform corpus every reference performs the same, at 277.1, 256.0, 277.7 and 277.1. At maximum entropy the choice of \(Q\) stops mattering, because \(D(P\|P) = 0\). The policy sweep in Section 4.3 reaches the same conclusion from the other side, removing the reference entirely instead of replacing it, and the uniform corpus again refuses to separate.
...
Rejection rate and advance distance are two factors of one quantity. A read at offset \(a\) returning symbol \(c\) advances by \(\mathrm{adv}_a(c)\). An anchor is worth
...
Which of those two happens is decided in advance by one number. Modeling the needle as carrying the observed symbol at rate \(q = 2^{-H_2}\) per position, the distance to the nearest earlier occurrence has \(\Pr[d > j] = (1-q)^j\),
...
\noindent\textbf{The consequence for a search that gives up the order.} For large \(m\) the harvest tends to \(2^{H_2}/m\). The ceiling on what dropping the constraint can buy is \(m\,2^{-H_2}\), the expected number of times the observed symbol occurs in the needle. That is roughly 20 on English prose at \(m = 256\) and 1.07 on the uniform corpus at \(m = 16\). The regime where order costs something is low entropy and long needles, and there is nothing to collect at high entropy and short ones.
...
If order is free then a read is not attached to an anchor at all. One read of one cell refutes every alignment whose needle position holds a different symbol. There is nothing to choose except which cells to read, and those can be fixed in advance. Reading at stride \(k\) covers each alignment \(m/k\) times. With \(x = m/k\) the survivors are \(N 2^{-H_2 x}\), the probe reads are \(Nx/m\), and the total is minimized at
...
Measured, as in-order reads over free-order reads. Above one is a saving:
...
The dependency depth is 2 on every row, as the construction requires. It is the depth of \texttt{anchor\_sift\_free}, where every probe issues at once and one branch is taken on the combined result (\texttt{anchor\_sift.h:155-157}), and it is unrelated to how many levels a descent runs (Section 2.7).

\noindent\textbf{The probe phase behaved exactly as modeled and the model still failed.} Survivors after probing at \(m = 256\) are 1, 7, 8, 11 and 2 across the five corpora, against a prediction of 1.25 to 3.84. The reads went somewhere the model never counted.
...
\noindent\textbf{A long search is verification bound and not filter bound.} The floor is \(m\) reads for any algorithm that must confirm the match, and at \(m = 256\) Horspool runs at 1.10 to 1.57 times that floor while free order runs at 1.04 to 1.16. Both are already near a bound that no filtering strategy can move. An order of magnitude was never available. The saving that does exist is the gap between those two ratios, and 1.47 at \(m = 128\) on English is the largest of it measured.
...
\noindent\textbf{The accumulation is bitwise, and that makes the count reachable.} A refutation set is a set of alignments, sets union commutatively by Proposition 1, and a set of alignments is a bit vector. accumulating a read costs \(\lceil m/W \rceil\) word operations for a machine word of \(W\) bits, the surviving set is the complement, and no other structure is needed. At \(m = 256\) with 64 bit words the whole probe phase is 25 reads and 100 word operations, at a dependency depth of two, against 385 strictly sequential reads.

Each mask is indexed by pattern position, which makes it \(m\) bits wide whatever the alphabet is and whatever the dimension is, and the alignment vector the masks clear grows with the field (Section 2.7). Section 2.1 can therefore decline to bound the alphabet or the dimension at no cost: neither appears in the state.
...
The calibrated stride and the derived stride agree closely, 98 against 91 and 75 against 84 and 44 against 45. The failure is not a mis-set constant. The safe stride is a property of the pattern and the corpus together: a pattern carrying rare symbols tolerates a long stride and one carrying common symbols does not, and a measurement on one pattern bounds nothing about another. One known constant calibrates one pattern.

\noindent\textbf{What the failure was, since it was predicted to be something else.} This section originally reported the model's 11 to 26 times as its result, flagged as resting on the independence assumption that Section 4.6 measures failing worst on \texttt{structured}. That flag was aimed at the wrong thing. The independence assumption held: predicted survivors 1.25 to 3.84 against measured 1 to 11. What the model omitted was the \(m\) reads needed to confirm the occurrence every needle is guaranteed to have, a term with nothing probabilistic in it.
...
\noindent\textbf{The selecting phase is small without being made small.} Nine probes settle \texttt{structured} at \(m = 256\), against \(\log_2 256 = 8\), the size deterministic sampling reaches by analyzing the pattern first. English and uniform take 23 to 27. Nothing here derives a sample, carries state between searches, or looks at the pattern before the first read.
...
Proposition 1 does not mention the width. Soundness holds at every width at once. What the width moves is cost. A read of \(w\) bits costs \(w\) bits of reading and yields \(H_2(w)\) bits of discrimination. What a search spends is \(w \log_2 N / H_2(w)\) and the quantity to compare across widths is \(H_2(w)/w\).

Measured over windows taken at every bit offset, since a read can begin anywhere (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}sift.\allowbreak{}c:1122}):
...
\noindent\textbf{The flat corpus is not flat.} It is \texttt{0x41} repeated, \texttt{01000001} has period eight, and a window of six bits or more takes exactly eight distinct values whatever the corpus length. Its collision entropy is therefore \(\log_2 8 = 3\) bits exactly. The sweep reports that at widths 6, 8, 12 and 16, and 0.678 bits at width one. The object every other section of this document reports zero on carries three bits under a slice one bit narrower.
...
Every measurement before this one is given a needle. The construction does not require one. A shift \(d\) is the hypothesis that the corpus agrees with itself \(d\) apart, a pair of read cells whose positions differ by \(d\) is a test of it, and one disagreement refutes it permanently. So the pattern is not one string, it is every shift at once, and the reads are shared across all of them: \(k\) cells carry \(k(k-1)/2\) tests. Here 512 reads test 130,816 pairs (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}ab.\allowbreak{}c:1341}).

\noindent\textbf{The threshold cannot be trusted and is not.} Tests of one shift share read positions and are not independent, and a standard error computed as though they were is too small. The first run of this reported twelve shifts on the uniform corpus, which is SHA-256 output and has nothing to find.

A shuffle of the same bytes settles it. It preserves the byte histogram exactly. The collision probability is unchanged, and it destroys every relation between positions, and that is all a shift can be. Whatever the detector reports there is what it reports on nothing.
...
One corpus has structure of this kind and it is the one built with a period. Its strongest shift is 560, and \(560 = 35 \times 16\). The detector recovered a multiple of the record length without being given the record length or a pattern to look for. The \texttt{mixed} corpus peaks at 416, ala multiple of 16, which is its periodic quarter showing through below the threshold.

\noindent\textbf{What it does not find is as informative.} English prose and C source report nothing above their shuffles, and English peaks lower than its own shuffle does. Both plainly have structure. What they do not have is agreement with themselves at a fixed offset. That is the only hypothesis this formulation tests. Repetition at irregular positions, which is what a word or an identifier is, needs a different query. The detector finds periodicity and is silent on everything else.

\subsubsection{4.12 Boundary Detection}

Section 4.11 asks whether a corpus repeats at a fixed offset, and prose does not. A different question reaches it. Whatever else a language is, it composes units. Something in it marks where one unit ends, by a separator or by a rule. If nothing does then nothing is a unit. That has a signature the same reads can measure.

If a symbol's positions carry no structure, the gaps between its occurrences are geometric. The ratio of the variance of the gaps to their squared mean sits near one. A separator has bounded unit length. Its gaps are far tighter and the ratio falls. The statistic has no units and no scale. One threshold reads the same on any alphabet and any corpus (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}ab.\allowbreak{}c:1502-\allowbreak{}1508}). The shuffle calibrates it as before: it holds every symbol's count and destroys where they sit, which makes every symbol's gaps geometric by construction.
...
Nothing here is given a pattern, a dictionary, or a notion of what a unit is. What makes the comparison sound is that both objects are finite and hold the same symbols. The null is constructed and not assumed.

\paragraph{4.12.1 Boundary Spacing as a Filter}

A known boundary symbol turns a corpus into a sequence of gaps, and a needle carrying \(k\) boundaries carries \(k-1\) gaps between them that any true occurrence has to reproduce. That run is a filter on its own, before any other symbol is looked at (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}ab.\allowbreak{}c:1639-\allowbreak{}1650}).
...
\noindent\textbf{The record corpus is the counterexample and it does not move.} Its reduction is exactly 16.0 at every needle length and at every boundary count, because its newlines are perfectly periodic: the spacing fixes the alignment modulo 16 and says nothing further. A boundary of period \(T\) carries \(\log_2 T\) bits and no more, however many of them a needle holds.
...
\noindent\textbf{The Finnish row is a genre confound and not a property of Finnish.} Every other text here is running prose and the \emph{Kalevala} is verse in strict trochaic tetrameter, compiled from oral songs. Its lines are more regular than its words and the detector finds the line. Replacing it with Finnish prose settles it:
...
A second objection is stronger than the statistical one. A drift floor is a claim about a population living out normal lifespans in one place with nothing interrupting the chain of transmission, and the only genre matched pair available spans the English Civil War, the regicide, the plague of 1665 and the fire of 1666. That interval is the opposite of the condition. 0.5625 is not a floor. Read as an upper bound on how fast the common vocabulary moves, it says roughly 72 of the 100 most frequent words survived that half century unchanged.
...
That gap is where a language replacement sits. A change at the edge of a civilization substitutes one language for another instead of altering a rate, and substitution is discrete, a mechanism for changes arriving in groups that does not require the drift rate to vary at all. Measuring it needs a replacement event with text on both sides, and the drift rate is the only quantity these measurements compute. For the oldest replacement events there is no text on either side and there will not be one. Writing is one channel among several, and this document cannot read the others at all. A gap in the written record across an event is not evidence that continuity failed there.

There is a selection effect in this that no corpus work removes. The events most likely to show changes arriving in groups are the events that leave nothing to read, and the intervals that can be measured are the ones mild enough that a writing tradition survived them. A corpus of written language is a sample of the surviving cases, and it is biased against the phenomenon being asked about.

A pattern can be certainly present and its meaning permanently unrecoverable. That is Proposition 1 holding while Proposition 2 refuses. The filter of Section 2, run backward on a real object, shows it.

An Attic black-figure amphora signed by Exekias, Achilles and Ajax at a board game (Vatican inv.\ 16757, 540 to 530 BC; the photograph is by Daderot, released CC0), was read at 160 by 107 gray levels, copied fifteen times, and each copy had 40 percent of its pixels replaced by a value from nowhere. \texttt{examples/\allowbreak{}art/\allowbreak{}4\_measure/\allowbreak{}classify\_reject\_recover.\allowbreak{}py} (ART-4-007), told only that the subject repeats, recovered the image by per-pixel consensus. All 17{,}120 pixels came back bit-exact, a noise reduction ratio of 100 percent, and the filter flagged 896 of them, 5.23 percent, as a floor it would not certify because its two exact routes disagreed there. Not one flagged pixel was wrong and not one wrong pixel went unflagged. Figure~\ref{fig:ajax-recovery} shows the clean image, one damaged copy, the recovery and the uncertainty map.

The filter recovered the object to the bit and recovered nothing of what it meant. Ajax, Achilles, the game between them and the throw Achilles calls out are not in the pixels, and no consensus returns them. The pattern is present and recoverable to the last bit, and its meaning is not the filter's to give. That is the asymmetry this section reports, on a real object instead of a ruin.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{ajax_recovery.png}
\caption{A real Indo-European object recovered to the bit. Left to right: the Exekias amphora detail read at 160 by 107 gray levels; one of fifteen copies with 40 percent of its pixels replaced by a value from nowhere; the per-pixel consensus recovery, bit-exact on all 17{,}120 pixels; and the uncertainty map, 896 pixels (5.23 percent) flagged as the floor the filter would not certify. The signal returns exactly and its meaning does not. Photograph by Daderot, CC0; recovered by \texttt{classify\_reject\_recover.py} (ART-4-007) and reproduced by \texttt{evidence/\allowbreak{}proofs/\allowbreak{}posits/\allowbreak{}recover\_indoeuropean\_art.py}.}
...
One caution belongs with this and it is now measured. Distributions of this shape alarise from processes doing no optimizing at all, and Section 7.4 runs that counterexample against these same measures. Independent character draws carrying a delimiter return a Zipf slope of -0.988, inside the range every natural corpus here occupies. The Zipf slope in this section carries no evidence about production and is withdrawn as such. The brevity correlation survives weakened, at -0.222 for the control against -0.247 to -0.364 for natural text. The boundary regularity against a null permutation survives intact, at 1.00 for the control against 1.79 and above for natural text. That counterexample does not weaken the method in this document, and strengthens it. A structure produced by nearly any generator is a better thing to key on than one belonging to a special class of them, and the requirement here was never that the source be a language. What the measures here need is that something regular produced the bytes. The filter of Section 2 needs less, a decidable equality between positions (Section 2.1), so it needs no alphabet, no dimension and no interpretation.
...
The corresponding claim about the rare half is that it carries the actions, which needs counting. No part of speech tagger is available here. English verb inflection stands in for one, and the proxy is weak in one direction: a gerund used as a noun and an adjective built from a participle both carry these endings without being verbs. Every figure is an upper bound (\texttt{examples/\allowbreak{}language/\allowbreak{}4\_\allowbreak{}measure/\allowbreak{}head\_\allowbreak{}tail\_\allowbreak{}parts.\allowbreak{}py:31-\allowbreak{}36}).
...
Scoring individual words for clustering against the same null permutation (\texttt{examples/\allowbreak{}language/\allowbreak{}4\_\allowbreak{}measure/\allowbreak{}word\_\allowbreak{}burstiness.\allowbreak{}py:44-\allowbreak{}72}) answers two questions, and only the second of them comes out.
...
Hugo is the lowest of the single works and that supports the reading instead of straining it. \emph{Les Misérables} runs to five volumes and turns aside into Waterloo, the sewers of Paris and the history of a convent. It is the most topically varied single work measured here and it sits nearest the collection band.
...
\noindent\textbf{Zipf is unchanged to two parts in a thousand}. A property of the message has to do that under a relabeling of its symbols.

\noindent\textbf{Brevity is nearly unchanged, and that refutes the reading offered in Section 4.13.1.} A permuted table keeps a correlation of -0.550. At most 0.10 of the -0.652 can be credited to Morse assigning short codes to frequent letters. The rest is structural: short strings recur more often because there are fewer short strings, and that holds for any table whatever. The stronger brevity in the Morse rows than in the English row is mostly an artifact of the unit length distribution and not evidence of design.

\noindent\textbf{What the design does buy is the length.} The real table encodes the same text in 22.4\% fewer bytes than a permutation of its own codes. That is Morse's engineering, and it appears in transmission cost and not in the correlation it was attributed to.
...
rare half against a null permutation & 0.0731 & 0.0342 & 0.66 & 0.68 \\
\bottomrule
\end{longtable}
\normalsize
...
What this document adds has to be stated carefully, because two different instruments appear in it and they have been run together in earlier drafts. The dispersion measure used here has been applied to text, to source code and to recorded sound, which are unlike media, and it separates an arranged domain from an unarranged one in each. The dispersion measure has not been applied to pictures or to the crystal lattices, which were measured by the shift detector instead, and that is a separate instrument with a separate null.
...
Narrow company catches connectives along with subjects, and a connective is the wrong answer twice over: it is needed everywhere. It says nothing about what a text is about, and its form varies between languages while its job does not. Where a unit falls separates them. A unit doing grammatical work is spread evenly. The gaps between its occurrences are near geometric and their dispersion sits near one. A unit belonging to a subject arrives in bursts where that subject is discussed, and its dispersion runs far above one. That is Section 4.12's statistic read at its other end (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}ab.\allowbreak{}c:2210-\allowbreak{}2217}).
...
\noindent\textbf{The size of the numbers carries something as well.} The bible bursts between 6.6 and 20.8 and the novel only between 2.5 and 4.3. A compilation of books written centuries apart has vocabulary that clusters hard by section, and a single novel is homogeneous enough that little clusters at all. The scale of the statistic reports how uniform a text is.

\subsection{5. Discussion}

\subsubsection{5.1 Algebraic Structure of the Filter}

Strip the cost away and what remains is small. The state is one bit for each alignment, alive or dead. A read of one cell selects a mask over the pattern's positions and clears the alignments whose position holds something else. Survivors are the complement. That accumulation contains no comparison that steers control flow, no position computed from a value just read, no ordering on positions, no metric, and no dimension. A search that chooses its reads, as Section 4.9.7 and the engine's descent do, adds comparisons of survivor counts that steer which cell is read next and whether another is read, and the count after the exact compare is the same whatever they choose (Section 2.7).

Proposition 1 is why the pieces commute: a refutation depends only on the cell that produced it, and a set of reads contributes a union and a union does not remember the order it was built in. The consequences are all mechanical from there. Reads can be issued together because none waits on another. Accumulation is a bitwise OR. The surviving set is a complement. The mask is \(m\) bits wide whatever the alphabet holds and however many dimensions the index set has. Neither quantity appears in the state and neither needs a bound. The alignment vector grows with the field, so the space bounds proved for streaming matchers do not describe it (Section 7.5).
...
A single scalar fixing a cost surface across every dimension and every alphabet is qualified in three ways, and the next three sections state them.
...
Those are four faces of one thing. The parameter is a marginal and each failure is a statement about a joint. The boundary is not an accident of these corpora. It is where a marginal stops being able to say anything.

\subsubsection{5.6 Transferable Results}

The product rule failure is the portable one. Any structure that stacks cheap filters and sizes itself by multiplying their rates inherits it on structured data, in the same direction, and under-provisions. That transfer is a conjecture: the failure is measured only on anchor cascades, here and in the engine workbook (\texttt{theory/\allowbreak{}workbook/\allowbreak{}chapters/\allowbreak{}chapter\_anchor\_sift\_workbook.tex:296}), and on no other filter structure.
...
Symbols are drawn from an alphabet of at most 256 values in every bench, and the complex domain uses two. A claim that the construction tolerates an unenumerable alphabet is supported here by the core never reading a value (\texttt{bench/\allowbreak{}driver/\allowbreak{}bench\_\allowbreak{}ancorae\_\allowbreak{}lattice.\allowbreak{}c:80}, \texttt{193-\allowbreak{}239}) and by the engine's interface to a symbol being equality alone (\texttt{anchor\_sift.h:354-357}). No measurement supports it, because a measurement on an unenumerable alphabet cannot be performed. A growing alphabet is measured instead: from \(2^8\) to \(2^{24}\) symbols the memory of the engine's equality route stays at 131057 bytes while a table indexed by symbol grows with the alphabet (\texttt{docs/\allowbreak{}findings-\allowbreak{}for-\allowbreak{}verification.md:263-\allowbreak{}274}). Past 256 classes the engine's rank count is an upper bound (Section 2.7).

\subsubsection{6.3 Finite Precision}

The complex domain's symbols are square roots of primes held in \texttt{double}, which makes the values stored rational approximations of irrationals. Equality is decided over storage. The approximation affects which symbols are distinct and never whether the comparison is sound.
...
Two of the three constants are exponential in \(H_2\). That error does not stay small:
...
Any vectorized cost model. Every figure in Section 4.9 counts sequential reads, and a shift is inherently serial because the next position cannot be computed until the current one has been read. Under a model that tests many positions per word operation, the advance advantage that decides Section 4.9.1 may not be available at all, and rejection would be the only lever left. The engine's only wide path is the planner's survivor scan (Section 2.7), which is not this arrangement, so that measurement would still decide whether the arrangement in Section 4.9.3 belongs in the library, and it has not been made.
...
The arXiv preprint at \texttt{2601.\allowbreak{}03271} selects a single anchor, \(a = \arg\min_i \mathrm{freq}(P[i])\), and reports character comparisons on the \emph{Divina Commedia}. Having one anchor, it contains no separation, independence or joint probability treatment, and uses no periodic or fixed width data.
...
\noindent\textbf{What that leaves, stated more carefully than a first reading of it allowed.} The prior art above is combinatorial and worst case. It selects a rarer symbol without pricing what rarity is worth, and it carries no distribution over the alphabet.
...
Read from Tsai, \emph{Average Case Analysis of the Boyer-Moore Algorithm}, Random Structures and Algorithms 28, 481 (2006), pages 1 to 4 and 14 to 16 of 18. Sections 4 and 5, which carry the Markov chain derivation, were not read.
...
\noindent\textbf{The parameter is alphabet cardinality.} The text is written over ``a set of \(q\) characters'', every numeric result is computed at \(q = 2\), 4 and 8, the error term is \((m-K-1)/q^K\), and the experiment uses text ``built randomly''. Under a uniform alphabet \(\sum_\sigma p_\sigma^2 = 1/q\) exactly. \(2^{H_2} = 1/\sum p^2\) is the effective alphabet size and the collision probability used throughout this document is that \(q\) carried to the non-uniform case. The transition probabilities in their framework are written for a general distribution. The generalization is available there and is not taken. This document does not claim a new quantity, it claims the classical one at a weaker assumption.

\noindent\textbf{A comparison count cannot see a declined refutation.} Section 4.9.4 prices what an in-order shift leaves uncollected, between 3\% and 99\% depending on the needle length. That quantity is invisible to \(C_n\), because a comparison count records what the algorithm did and a forfeited refutation is something it never did. The gap is not a defect in thirty years of analysis. It is outside what the metric measures, and it becomes visible only once the traversal order is treated as a free variable, which Section 7.1 credits to Vishkin.

\subsubsection{7.3 Thermodynamic Information Bounds}

Sections 4.3 and 4.8 reach for a physical analogy: a reference measure is worth the divergence of the domain from it, and at equilibrium there is no gradient and nothing to extract. Read from Parrondo, Horowitz and Sagawa, \emph{Thermodynamics of information}, Nature Physics 11, 131 (2015).
...
\noindent\textbf{What this costs Section 4.13, measured and not conceded in the abstract.} Sproat's counterexample class was built and run through the same code that produced every other row (\texttt{maint/\allowbreak{}data/\allowbreak{}fetch/\allowbreak{}gen\_\allowbreak{}monkey\_\allowbreak{}corpus.\allowbreak{}py}). Characters are drawn independently and a delimiter is dropped in at a fixed rate.
...
dispersion against a null permutation & 0.99 to 1.01 & 1.79 to 4.82 & yes \\
\bottomrule
\end{longtable}
\normalsize
...
One reading survives this and is not tested by it. Both corpora that score away from the null were written by people, which leaves the result consistent with the measure detecting production by a person while failing to distinguish which kind. Separating those needs a system carrying structure that no person produced, of the sort Rao and colleagues reach for with genomes and protein sequences. No such control is measured here. No claim of that kind is made.

\paragraph{7.4.4 Scale Localization of Structure}

The null used above shuffles single symbols, which destroys every arrangement at once and cannot say which span an arrangement occupied. Cutting the corpus into blocks of \(B\) symbols and shuffling the blocks keeps everything shorter than \(B\) and destroys everything longer (\texttt{data/\allowbreak{}block\_\allowbreak{}shuffle\_\allowbreak{}null.\allowbreak{}py:52-\allowbreak{}59}). Sweeping \(B\) locates the signal.
...
That is a statement about the clustering measure and not about the text. The engine workbook measures anchors inside a needle of 24 symbols surviving above the product of the anchor rates, at a median ratio of 3.55 over 60 needles on English prose (\texttt{theory/\allowbreak{}workbook/\allowbreak{}chapters/\allowbreak{}chapter\_anchor\_sift\_workbook.tex:296}, \texttt{:323}), which is correlation between positions closer together than 24. The 20992 once quoted here is that workbook's sizing error under the rarest symbol rule (\texttt{:331}). The workbook records that means of this heavy tailed ratio were quoted as typical values (\texttt{:600}), and it still prints 20992 at \texttt{:331}, \texttt{:1162} and \texttt{:1164}. Both hold. The block sweep says the clustering signal has no component below a span of 32, and the cascade says a different arrangement lives below 24 that the clustering measure cannot see.
...
\noindent\textbf{The same finding points the other way for the method.} Section 4.13.08 argues that these regularities are regenerated by the conditions that produce a message and are not carried between instances. Sproat's position is the stronger form of that: the structure arises from generators with no communicative intent at all, dice included. A method keying on a property that nearly every generator produces has a wider domain than one keying on a property special to language, and Section 2.1 never required the source to be a language. Section 2.1 assumes a decidable equality between positions, so the filter needs no regularity in the source. The measures in this section need something regular to have produced the bytes.

\subsubsection{7.5 Filtering, Coverage and Streaming Bounds}

Navarro, \emph{A Guided Tour to Approximate String Matching}, ACM Computing Surveys 33(1):31-88 (2001), requires every filter to be coupled with a verification of the positions it could not discard, which is Propositions 1 and 2 for edit distance. The engine workbook records that reading and its quotations (\texttt{theory/\allowbreak{}workbook/\allowbreak{}chapters/\allowbreak{}chapter\_anchor\_sift\_workbook.tex:1208-\allowbreak{}1214}).

Nemhauser, Wolsey and Fisher, \emph{An Analysis of Approximations for Maximizing Submodular Set Functions, I}, Mathematical Programming 14:265-294 (1978), doi:10.1007/BF01588971, give the \(1 - 1/e\) bound for greedy maximization of a monotone submodular function under a cardinality constraint, which Section 2.7 applies. The bibliographic record was checked and the paper was not read, so the bound is used in its standard statement.

Clifford, Jalsenius, Porat and Sach, \emph{Space Lower Bounds for Online Pattern Matching}, CPM 2011, arXiv:1106.4412, prove \(\Omega(m)\) bits for the \(L_1\), \(L_2\), \(L_\infty\), Hamming, edit and swap distances and for distance functions with wildcard-like properties, pattern matching with character classes among them, with each answer required correct only with constant probability. For other distance functions they give \(\Omega(\log m)\) and \(O(\log^2 m)\) bits, and exact matching is not among the \(\Omega(m)\) cases. Porat and Porat, \emph{Exact and Approximate Pattern Matching in the Streaming Model}, FOCS 2009, 315-323, match exactly in \(O(\log m \log n)\) bits with Karp-Rabin fingerprints, which are Monte Carlo. Both are cited from the engine's record (\texttt{docs/\allowbreak{}findings-\allowbreak{}for-\allowbreak{}verification.md:298-\allowbreak{}320}). Those models keep a state while a text passes and permit error. This engine's output grows with the field and its primitive is exact equality, which exposes no arithmetic to fingerprint.

Minsky, \emph{Recursive Unsolvability of Post's Problem of ``Tag'' and Other Topics in Theory of Turing Machines}, Annals of Mathematics 74(3):437-455 (1961), is cited only as the target of an encoding that is owed (Section 2.7).

\subsection{8. Conclusion}

The safety of a delta null permutation and the saving from it are different claims and were not
measured as such. The first needs no data and was confirmed over 9,396,207 true occurrences with
none refused. The second is a property of the specific domain, and moves by up to a factor of 6.4
between measures.

The engine carries the first claim with its conditions in Section 2.7: the count is exact for any
legal probe set once the full compare runs.

One quantity turned out to do three jobs. The collision probability \(2^{-H_2}\) sets the rate at
which an uninformed anchor admits a candidate, sets the fraction of alignments a single read
settles, and sets the size of the bias correction separating the two estimator forms. None of the
three were derived. All three independently measured and reverified as correct.

Three experimental results apply to the method:

\begin{enumerate}
  \item Candidate count is an estimator of collision entropy that needs no histogram or codebook.
    The accuracy costs 1 to 20 times against published estimators, depending on the probe budget.
  \item Sizing a cascade by multiplying candidate rates underestimates survivorship by up to four
    orders of magnitude on structured data, always towards scarcity.
  \item A delta null filter built and executed this way settles \(m\,(1 - 2^{-H_2})\) alignments
    per read, which is a property of the necessary condition of the term and not of any particular
    shift table.
\end{enumerate}

Where an order on positions exists, that order is most of the available structure and a shift takes it. The candidate rate this document spends its length measuring is not what governs a search. Section 4.9 records that as a loss of 1.2 to 25 times and then recovers it, first by treating rejection and advance as one product and then by measuring the advance directly. The arrangement that comes out ahead holds nothing. That is a product of the underlying assumptions about the set \(\Sigma\).

\subsection{9. Reproducibility}

The bench rows in this chapter were taken on four self contained programs and one cost profile, built with the line below. The line is kept as it was recorded. Its paths under \texttt{test/\allowbreak{}} and \texttt{src/\allowbreak{}impensa\_\allowbreak{}ancorae\_\allowbreak{}acus/\allowbreak{}} are not in anchor\_sift's history, and the four programs are at \texttt{bench/\allowbreak{}driver/\allowbreak{}} at commit \texttt{d65d219} (Section 1):
...
\texttt{bench\_\allowbreak{}lattice.\allowbreak{}c} does not parse under MSVC from line 500 (\texttt{docs/\allowbreak{}steering.md:280}). This chapter records no run of the benches as they stand at that commit.
...
\subsection{Appendix B: The examples tree}

Every domain enters at stage 1 and leaves at whichever stage it has an answer for.

The examples under \texttt{examples/\allowbreak{}} are 122 scripts over nine domains, each one a
stage of the same pipeline. A domain enters at stage 1 and leaves at whichever stage it has an
answer for.
...
\midrule
\texttt{1\_represent} & what counts as a part, and how a part is written down \\
\texttt{2\_partition}  & how the parts are cut into the cells a distribution is taken over \\
\texttt{3\_reference}  & the maximum entropy arrangement the object is measured against \\
\texttt{4\_measure}    & the departure from it \\
\texttt{5\_sift}       & what the departure admits or refuses \\
\texttt{6\_oracle}     & the outside answer, where a domain has one \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
domain & scripts & reaches \\
\midrule
\texttt{language}        & 60 & oracle \\
\texttt{any\_corpus}     & 18 & sift \\
\texttt{crystallography} &  9 & oracle \\
\texttt{proteins}        &  8 & oracle \\
\texttt{cell\_tracking}  &  7 & oracle \\
\texttt{art}             &  7 & measure \\
\texttt{game\_theory}    &  6 & oracle \\
\texttt{source}          &  4 & measure \\
\texttt{sound}           &  3 & measure \\
\bottomrule
\end{tabular}
\end{center}

Five of these domains carry an oracle stage, and the rest stop short of one. This book's results
rest on two oracle answers this work did not produce: crystallography's published cell edges and
language's hand extractions. The proteins and game theory oracles test against published values as
well, and those comparisons belong to their own theories. Everything this book measures beyond the
two named is taken against its own null permutation and says so.

% A Markdown horizontal rule that came through the conversion. LaTeX set it as a stray em dash
% floating above the attribution.
\medskip
