...
\section{The window limits which rows can be scored}

The detector scores a candidate period together with its own multiples. The crystallography chapter
made that correction when published cell edges came back at twice their value on ten of eighteen
axes. A period beyond half the window has one multiple inside the window. Its family is then one lag,
and the score degenerates to taking the tallest lag again.
...
A shuffle keeps every Grundy value and destroys which position carries it. Whatever margin it
reaches is what the detector reads off the histogram alone. The figure quoted is the tallest of the
twelve, not their average.
...
that a coincidence would have to be systematic. Luck does not reach three hundred and eighty three.
...
to the domain's distribution. A result obtained inside one distribution stays inside it. A
subtraction game's Grundy sequence is exactly periodic after a short preperiod, with two to a few
dozen distinct values and no noise at all. That is the easiest case a period detector can be shown. The series establishes that the detector is right where an answer exists and is checkable,
