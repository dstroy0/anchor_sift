...
Her own reading of the story is in build/audio. It is an oracle for the extraction and not only a source. The coverage check applies a cruder space closer here than the oracle check does, the other drift named above.
...
Every form is cited from a published dictionary of one of eighteen languages. There is nobody this corpus is of. The who column carries the language instead, and the reader writes a .pure.tsv keyed by it and no flat pure file.
...
William W. Elmendorf, Word Tabu and Change Rates, ICSNL 2. Thirteen languages are cited in it and Twana is its subject. That is the language named here; every row carries its own language in the who column. No speaker is named anywhere: the forms come from Boas and Haeberlin 1927, Krueger 1967, McIlwraith 1948, Walters 1938 and Ray 1932, from Warren Snyder's Suquamish list and Wayne Suttles' Squamish field notes, and from Elmendorf's own field data. This typescript writes its uvular as x under a dot where 1975\_Hilbert\_Hess writes it under a caron, and 1967\_Hess agrees with it, which is two papers of one conference against one of a later year. One mark is unresolved and marked so in the table: a short raised stroke over the s of the Columbia ska'u on page 7, which is not the wedge the Upper Chehalis sča'u carries two words earlier.
