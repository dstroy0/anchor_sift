...
developing organism. When that surface is close to a sphere it is modeled as an evolving sphere-like
...
The radius is fitted to sample points on the surface, the approximate centers of the tracked
objects, after centring them on a fitted sphere. The fit minimizes a data term plus a Sobolev
seminorm penalty,
\[
...
basis, weighting degree \(n\) by \((n(n+1))^s\). Smoothness is a statement about how fast coefficients
...
expansion is therefore a Helmholtz-Hodge decomposition already. That is why vector spherical
...
The optical flow on the surface minimizes a data term, the squared residual of the generalized
...
carry the large-scale flows of the surface, the waves that move whole neighborhoods of objects
together. What no low degree explains is the motion particular to one object. A tracker that removes
the low degrees before it links is linking on that remainder, the order the first chapter
asks for.
