...
statement point by point. \textbf{Scope:} Fefferman's Clay problem description, 6 pages;
\emph{OpenAI 2026 Navier-Stokes}, 166 pages; \emph{OpenAI 2026 Euler}, 57 pages; the Lean~4
repository \texttt{openai/\allowbreak{}NavierStokesAndEuler}, four source files of 2{,}669 read,
one of them the adapted \emph{Formal Conjectures} statement.
...
A smooth external force is therefore named inside two of the four statements, and a proof of any
one of the four is what the problem asks for. The only occurrence of the word \emph{prize} in the document
...
\textbf{The document carries errata and one of them adds a condition.} The first reads that the further
...
that gap and says so in the sentence after the theorem, asserting that there is then no smooth
solution on $\R^3\times[0,\infty)$ with the same force and initial datum whose kinetic
...
statement anybody meant. A machine-checked proof of a neighboring theorem compiles exactly as
...
translation. No proof assistant checks a translation.
...
$[0,\infty)$. Fefferman states equation (1) on the closed half-line, and the Lean encoding follows
him there. Divergence is defined as the trace of the Jacobian and carries an explicit junk value of
zero at points of non-differentiability, which the source declares instead of leaving implicit.

\section{The statement is not the authors' own}
...
tool under a sandbox, using an independent exporter and an independent kernel instead of Lean's
own.
...
and the axiom base is printed for a reader to check. The translation is still the thing to read, and
this chapter read it, but the party that wrote it had no stake in what would later be proved
against it.

\section{The unforced Euler companion}

A second paper was published alongside, \emph{Finite time blowup for the Euler equation}, 57 pages,
and it matters here for one reason: it is unforced.

It studies the unforced incompressible Euler equations on $\R^3$, $\partial_t u + (u\cdot\nabla)u +
\nabla p = 0$ with $\nabla\cdot u = 0$, taking initial data in the smooth compactly supported
divergence-free class and writing $T(u_0)$ for the maximal lifespan of the smooth solution. Its
Theorem 1.1 asserts that there exists such a $u_0$ with $0 < T(u_0) < \infty$, whose smooth
solution satisfies $\limsup_{t\to T}\|u(t)\|_{L^\infty} = \infty$ and
$\int_0^T \|\operatorname{curl} u(t)\|_{L^\infty}\,dt = \infty$.

The second divergence is not decoration. The Beale--Kato--Majda criterion, which the paper states
and attributes, says a smooth three-dimensional Euler solution continues past $T$ whenever that
integral is finite. Finite-time breakdown therefore requires it to diverge, and the theorem
delivers exactly the quantity the criterion demands. By the authors' own account the Euler result
came first, as the easier problem, and was used to prompt the Navier--Stokes effort.

\textbf{This is not a Millennium result and is not recorded as one.} Fefferman's statement says the
problems are also open and very important for the Euler equations at $\nu = 0$, and that the Euler
equation is not on the Clay Institute's list of prize problems. The distinction is his and is kept
here.

\section{What the unforced Euler result does not settle}

The reason to hold this paper is a question put to this work directly: what is left as the forcing
goes to zero? The test behind it is whether a mechanism repairs a property of the object or a
property of the setup, and a mechanism that needs the force to be there has taught us about forced
systems and nothing about the unforced problem.

The Euler paper looks like the answer and is not. \textbf{Removing the force and removing the
viscosity are different removals.} Statements (A) and (B), the two alternatives that remain open on
anybody's reading, are $\nu > 0$ with $f$ identically zero. The Euler equations are $\nu = 0$. So
an unforced blowup at zero viscosity does not transfer, and it does not transfer for the precise
reason that the regularity problem exists: the viscous term is the smoothing whose sufficiency is
the entire question. A construction that blows up with no viscosity to fight is not evidence about
what happens when there is.

Put as the limit test, the two papers move in orthogonal directions. One removes the viscosity and
keeps nothing to resist the amplification. The other keeps the viscosity and supplies a force. The
open cases keep the viscosity and remove the force, and neither paper is that case.

\textbf{That is an argument from the shapes of the statements and not from their mechanisms.} The
amplification mechanism was not read in either paper, and the honest version of the limit test
requires reading it instead of reasoning from the abstract. What is recorded here is that the
question is still open and that the obvious shortcut to it does not work.
...
placeholders rests on reading its source text. The check in this chapter
...
lemma the discharge module applies was not traced into the development. Of the Euler paper's 57
pages, only the abstract, the introduction and Theorem 1.1 were read; its amplification
mechanism, the part the limit test actually needs, was not. Nothing in this chapter rests
on the correctness of either proof, and nothing in it could, because neither was read.
