...
\texttt{PQC/\allowbreak{}examples/\allowbreak{}proofing/\allowbreak{}bbp\_\allowbreak{}scaling.\allowbreak{}py},
\texttt{PQC/\allowbreak{}theory/\allowbreak{}PARTITION.\allowbreak{}tsv}.

\textbf{Whose this is.} The recommendation in this chapter is the project architect's, given on
2026-09-11 in answer to a direct question,. It is not
...
reproduces existing work at a worse scale. The conjecture is not precision-bound. An engine
...
first $10^{13}$ of them was supplied with the recommendation. The
argument does not depend on it, because it turns on the conjecture not being precision-bound,
whatever height has been reached.

\section{The recommendation}

Birch and Swinnerton-Dyer, on the ground that its obstruction matches the instrument, where the
other Millennium problems match only in subject.
...
The withdrawal is the substantive part.

\textbf{Withdrawn.} That a real number carrying no integrality structure has its distance to the
nearest integer distributed uniformly on $[0,\tfrac12]$. That landing within $10^{-N}$ of an
...
One narrower objection raised against it was itself wrong. The suggestion
...
spread of the residual about the true integer. That spread is the instrument's own floor, and the
period and the regulator will dominate it well before the arithmetic does. The second is the
positive control and the floor measurement in one pass.
...
\texttt{bbp\_scaling.py} in the PQC tree measures what depth costs in the integer relation
search, and its recorded results were read in the file instead of taken on report.

Cost goes as precision to the 2.64. That is the measured exponent. Predicted was 2.585, from a
Karatsuba host multiply near width$^{1.585}$ against a count linear in the width. Blame does not
fall on the reduction.

Where the card overtakes the host: 1{,}024 limbs. Call it 32{,}768 bits. Mean operand runs 0.642 of
the precision. The mean multiply arrives at that crossover somewhere near 51{,}000 binary places.
One cell at 51{,}036 places, by the file's own table, costs 26.2 hours.

None of that binds the recommended measurement, which wants tens to hundreds of digits. Two other
things it does bind. Relation detection at tens of thousands of digits is off the table. So is any
plan resting on the card to rescue it. The file carries a note that the prediction ran the other way
...
The arithmetic the recommendation depends on lives in the PQC tree and is classified EXTERNAL
...
into held material and stops for a ruling.
