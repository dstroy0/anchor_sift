...
Degree is proved. Nothing here samples it. A Boolean function of degree below \(d\) has every \(d\)-th order derivative identically zero, and that derivative is the exclusive-or of the output over all corners of a \(d\)-dimensional input cube. One nonzero fold settles it.
...
The \(\geq 18\) entries are the cube ceiling of the bench. No measurement produced them. These numbers establish the separation. They do not measure the carry's absolute degree.
...
Choice and majority act inside the bit lattice, where \(\mathrm{GF}(2)\) describes them. A carry leaves its bit position, travels where \(\mathrm{GF}(2)\) has no coordinate, and rejoins one position higher. The measurement says that path outside the lattice carries almost all the degree.
