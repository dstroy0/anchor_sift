...
Eight words reproduces the wall of \(10\) measured independently by the depth bench, which is what makes the other rows readable.

\textbf{The offset is structure, not sample size.} The same table returns identical at \(2^{20}\), \(2^{24}\) and \(2^{28}\) pairs, a two-hundred-and-fifty-six-fold range with no drift. That is what a cliff predicts and a slope does not: a collapse completing inside one round cannot be pushed by adding samples.
