...
\textbf{Membership is the only relation asked of the set, so the search is a lookup.} A binary search over sorted coordinates answers the same question and imposes an order the set does not have, paying a full comparison at every step of it. The engine's C arms did exactly that until it was noticed.
...
The gridded reading set that bar at one voxel instead, because it had a voxel. It passed 453 of 453 and carried a bias of $-0.0067$ angstroms that it could not account for. The exact reading passes the stricter test and has no bias to explain. That is the same evidence, read without a floor under it.
