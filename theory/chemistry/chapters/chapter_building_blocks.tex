...
which names the element and orders the whole set; a count of valence electrons, which is the outer
...
lengths. Nothing about it is particular to chemistry until the values are filled in, which is why the
...
electrons because that is how many distinct states the quantum numbers allow it, so the capacities
are $2, 8, 18, 32$ and the period lengths follow from the order the sub-shells fill. This is not a
count anybody chose. It is fixed before any element is weighed, which is the property the later
