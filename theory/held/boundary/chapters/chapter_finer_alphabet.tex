...
The floor half of that is solid. Two independent states of the same weight give a level growing as the square root of the cell count, ordinary redraw statistics needing no assumption about the function being read. The delta half is only an expectation: a round's movement acts like a partial redraw and is expected to scale the same way. An attempt to derive that equality is recorded below, along with the reason it does not stand.
