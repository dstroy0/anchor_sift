...
A physical system forces a boundary reading: the interior cannot be reached, so it is inferred from what escapes. Almost nothing the engine is pointed at has that constraint, and treating it as though it does is a blindfold put on by hand.
...
Seven derived thresholds in one session and every one came in too low. The error runs in one direction only, so it earns a rule instead of more care: deriving a bar means enumerating the sources of variance, and the ones left out only ever \emph{add} variance, so the derived value is always the low estimate. A drawn bar has them all whether or not anyone thought of them.
...
\(\sqrt{2 \ln N}\), the \emph{expected} maximum, so half of all nulls exceed it \\
\bottomrule
\end{tabular}
\end{center}
...
\section{Normalise so the baseline is flat}

For points with no preferred direction every mode carries the same expected power, and a degree holds \(2l+1\) of them, so per-degree power \emph{rises} as \(2l+1\) with nothing in it at all. Drawn: the ratio is flat at \(3.867\) for a weight of \(64\) and \(5.095\) for \(128\), to within two and a half per cent across ten degrees, and the two constants stand in the ratio the finite-population correction predicts, \(1.318\) measured against \(1.333\) expected.
...
\begin{enumerate}
  ...
  \item Normalise until the baseline is flat.
