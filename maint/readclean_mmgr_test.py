...
"""Self-test for readclean_mmgr.py: what the blind pass removes, and what it must NOT remove.
...
# The only thing worth checking about a region macro is that it reads the offset it belongs to.
...
print()
print("the table is one per run, and a name crossing files stays itself")
bl = RC.Blinder(set())
