...
#
# Worth recording instead of quietly fixing: objective 5 moves the tools into tools/, which would
...
def layout_table():
    ...
    absent file returns an empty table, which the caller turns into a stopped run and not a
    silent one.
    """
...
# A file this tree produced and not wrote, announced by its own first line. Matched across the
...
def is_generated(path):
    ...
    Keyed on the file's first line and not a list of paths, because a path list is a second
