...
theory/workbook/*.md and theory/thought_experiments/*.md stay the source until the TeX port is
finished. Each one becomes chapters/chapter_<stem>.tex in its own book, and each book gets
chapters/order.tex, its list of chapters in the order theory/workbook/README.md names the files.
A file the README does not name comes after the named ones, by name. A PDF in a book's directory
is brought in whole with \\includepdf.
...
THEORY = ROOT / "theory"
BOOKS = ("workbook", "thought_experiments")
README = THEORY / "workbook" / "README.md"
...
def readme_order():
    names = []
    for match in re.finditer(r"([\w./-]+\.(?:md|pdf))", README.read_text(encoding="utf-8")):
        name = match.group(1)
...
def book_sources(book, held):
    ...
    ordered = ["README.md"] if book == "workbook" and "README.md" in present else []
    for name in readme_order():
        folder, _, leaf = name.rpartition("/")
        if leaf in present and leaf not in ordered and (folder or "workbook") == book:
            ordered.append(leaf)
