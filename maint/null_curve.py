...
def read_edges(path):
    ...
    with open(path, encoding="utf-8") as handle:
        ...
        for line in handle:
            ...
            row["body_voxels"] = int(row["body_voxels"])
    ...
    while draws <= most:
        ...
        for group, wanted in (("correct", False), ("failing", True)):
            ...
            for row in chosen:
                ...
                share += Fraction(delta, max(row["body_voxels"], 1))
...
def main():
    ...
    parser.add_argument("--history", default=os.path.join(root, "build", "tsv", "null_curve_history.tsv"))
    ...
    os.makedirs(os.path.dirname(os.path.abspath(options.history)), exist_ok=True)
