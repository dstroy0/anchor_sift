...
def main():
    ...
    floor_path = os.path.join(set_directory, sample, sample + ".knf")
    with open(floor_path, "rb") as floor:
        head = floor.read(16)
        assert head[:8] == b"KREP\0\0\0\0" and head[8:12] == b"KNF\0", "not a KNF krep"
        extent = struct.unpack("<4Q", floor.read(32))
        window, windows, _payload_crc, _cloud_crc = struct.unpack("<4Q", floor.read(32))
        _sample_root = floor.read(32)
        payload_at = floor.tell() + 8 * windows * windows
    assert window == WINDOW
    frames, depth, height, width = extent
    voxels = depth * height * width
    reader = zarr_frames.Frames(source, sample)
    assert list(extent) == reader.shape, "the noise floor's extent is not the source's shape"
    ...
    with open(floor_path, "rb") as floor:
        for voxel in chosen:
            for index in range(windows):
                ...
                floor.seek(payload_at + 8 * (index * voxels + voxel))
                held = struct.unpack("<Q", floor.read(8))[0]
