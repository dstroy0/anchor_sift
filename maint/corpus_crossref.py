...
#
#   python maint/corpus_crossref.py              all three passes
#   python maint/corpus_crossref.py --speakers   who the corpus rests on, and who is unnamed
#   python maint/corpus_crossref.py --forms      language forms only
#   python maint/corpus_crossref.py --prose      quoted paper text only
#   python maint/corpus_crossref.py --context N  lines counted as beside a use
#
# WHY THIS EXISTS
#
...
def run_forms(out, context):
    ...
    if not forms:
        ...
        out.write("  run maint/verify_private_sync.py\n")
        return 2
...
def run_prose(out, context):
    ...
    if whole:
        ...
        out.write("    a clean result covers those and proves nothing about the other %d\n"
                  % (whole - count))
        out.write("    python data/salishan/get_papers.py --all fetches them\n")
