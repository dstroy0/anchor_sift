...
# printed and exit 0, because the prose backlog predates the check and a hook nobody can satisfy gets
...
# ====================================================================
# THE SPELLING STAGE. BRITISH CONVENTION AGAINST AMERICAN, AS A PATTERN AND NOT AS A LIST
# ====================================================================
#
...
#
# This tuple was ten literals until now, and ten literals is what the sentence says not to write.
...
# braces defeats its stripper. 1,216 hits across the Python tree, of which 34 are identifiers rather
# than prose: `vonmises` and `cramervonmises` in scipy, `caretsloperise` in pygments, `sanssecours`,
# `miniser`, and one `suprises` that is a typo for `surprises`. Every other hit in both bodies is a
# British spelling in running prose. Call that 2.8 percent on somebody else's tree and 0.2 percent
# on this one.
#
...
_ISE_STEMS = (
    "advis", "revis", "devis", "televis", "improvis", "supervis", "chastis", "advertis",
    "exercis", "excis", "incis", "concis", "precis", "circumcis", "promis", "premis",
    "surmis", "demis", "compromis", "despis", "franchis", "merchandis", "paradis",
    "treatis", "expertis", "valis", "enterpris", "compris", "surpris", "appris", "repris",
    "upris", "sunris", "denis",
)
...
_OUR_WORDS = ("devour", "contour", "detour", "velour", "glamour", "paramour", "troubadour",
              "bonjour", "seymour", "hour")
...
LOCALE = LOCALE_NAMED + (
    ...
    # -re where American takes -er, as the named list the shape demands. The leading [A-Za-z]* is
    # what reaches `kilometres`, `millimetre`, `epicentre` and `multicentre`.
    ...
    # word boundary after `fulfil` is what keeps them out.
    ...
    # this arm reports the site and a person decides it, which is what a prose finding is for.
    ...
    # spelling in a way none of the arms above describes.
    ...
    # name spelled Grey is a false positive a person decides, which is what a prose finding is for.
...
BANNED = (
    ...
    # `so an` is the same construction and was unmatched until 2026-09-16. code-documentation:112
    # describes a CLAUSE -- "the personified consequence clause" -- and this pattern implemented a
    # TOKEN. Every `so a NN, down from NN` reported before this line changed measured the matcher
    # and not the tree: 21 of 105 instances across seven scoped ProtoCore files were `so an`, and no
    ...
    r"\bspelling\b",
    r"load-bearing",
    # The spelling stage, spliced in where its ten literals used to be hand-copied. LOCALE above is
    # the one copy. This is the position they held, kept so the order of BANNED does not move and a
    # pattern that overlaps a spelling still reports through whichever one BANNED reaches first.
) + LOCALE + (
...
# The spelling stage is defined above BANNED, because BANNED splices it in and a tuple has to exist
...
HUMAN_RATE = {
    ...
    # carry a rate to a population it was never taken over, which is the fault the rate exists to
...
# another 35. Every one is the standard writing out a token it bans so a reader can see which token
# is meant. A checker that reports a standard for naming its own bans is reporting the wrong thing
# 263 times, and it was.
#
...
# backticks anywhere goes quiet, which is the same trade QUOTED and PASSAGE already make. Bounded to
...
# was already in the spelling stage and was hidden by the extension list alone. The three
# `optimisation` sites at :305, :311 and :321 were hidden twice, by the extension list AND by the
# spelling stage being ten literals. An author who added the extension and not the pattern would
# have watched :123 fire, read the file as covered, and left the other three invisible 182 lines
# further down. Both halves of this pass are needed to see all four, which is why they are one pass.
...
# extension list cannot express `CMakeLists.txt`. Carried from repo_tools/docs/docs_maint/ai_words.py,
# which this pass supersedes.
BUILD_NAMES = ("CMakeLists.txt", "Makefile", "GNUmakefile", "Dockerfile")

# A git hook has no extension at all, which is the one selection rule no extension list can ever
# reach. Also from ai_words.py, and it is the sharpest thing in that file: a four-extension sweep
...
# is four places to forget it and four places for the four copies to drift apart, which is the same
# fault the note above LOCALE records about a rule table duplicated into the table that enforces it.
# There are three call sites and no others: prose_only applies the blanking rules once, so
# em_dashes, markdown_leftovers and banned_tokens inherit them without knowing they exist;
...
#
# NOTHING IS SKIPPED QUIETLY. A silent skip is how everything in this file got here. private_survey
# carries the canonical case in its own docstring: this scan covered zero closed repositories for
# the whole of a directory migration and nobody could see it, because "scanned none" and "there are
# none" printed the same nothing. So every exclusion below records what it dropped and why, into a
# Ledger, and main() prints that ledger under the counts. A file this tool declines to read is named
# by the run that declined to read it.
#
...
# measured cost and a test. A special case is a rule with none of those, and it is what the next
# person deletes.
...
#
# THREE INSTANCES, FOUND SEPARATELY, WHICH IS WHAT MAKES THIS A CONCEPT:
#
...
VERBATIM_ROOTS = (
    ...
    ("docs/learn/rfc",
     "the same IETF corpus, lowercased, in ProtoCore"),
    ("docs/learn/datasheets",
     "vendor datasheets and the .txt extracted from them"),
...
#
# SO A LISTED PATH IS REFUSED FOR REWRITING AND NEVER QUIETLY PASSED OVER, and a run that offered to
# rewrite anything in a tree carrying a manifest prints that manifest's own reconcile instruction
# under its output. The instruction is lifted from the manifest header rather than written out here,
# so it cannot drift from the tool that maintains it.
#
...
# in one is reported like any other, because reporting changes no bytes. Only the rewrite is refused.
...
def reconcile_command(manifest):
    ...
    try:
        ...
        pass
    return ("reconcile this tree against %s and re-sign it before committing"
...
def generated_regions(lines):
    ...
    for at, line in enumerate(lines):
        ...
        if hit:
            ...
            generator = named.group(1).strip() if named else (label or "an unnamed generator")
            opened_at = at
            continue
...
#
# BOUNDED TO THE ALPHABET TIER AND TO THE RUN, which is what keeps it from being a bypass. A
...
# is a live finding. Each arm names a word about writing: english, spelling, convention, usage,
# variant, orthography.
#
...
# maint/prose/test_docs_check_exclusions.py re-derives that zero from the trees on disk so a reader
# checks it instead of trusting this sentence.
BRITISH_SUBJECT = (
    re.compile(r"\b(?:british|american|canadian|commonwealth|oxford)\s+"
               r"(?:english|spelling|spellings|convention|conventions|usage|variant|variants"
               r"|orthograph\w*|dictionar\w*)", re.IGNORECASE),
...
#
# A rule that silences twenty correct findings and buys zero is a net loss, which is what precision
# over recall means when it costs something. So it moved to fix_refusal, where it refuses a rewrite
# on a line quoting a named standard and costs nothing at all.
...
CONTEXT_REASON = ("the subject of the passage is a writing convention, and a passage about one has "
...
def context_exempt(text):
    ...
    Returns a frozenset of tier names. Empty for every run in every tree measured today, which is
...
# shape of the sentence. TIER A AND TIER B ARE REPORT-ONLY, PERMANENTLY. This is not a feature
...
def fix_refusal(path, tier, at=None, regions=None, line=""):
    ...
    test leads because it is the one that holds everywhere and is never lifted.
    """
...
def git_say(where, args):
    ...
    if not os.path.isdir(where):
        return None
    try:
        answer = subprocess.check_output(["git"] + list(args), cwd=where,
...
# contains it.
#
# PRINT THE ROOTS SCANNED AND THE ROOTS LOOKED FOR AND NOT FOUND. private_survey already carries the
# reason in its own docstring and it generalizes: "scanned none" and "there are none" must never be
# the same output, for any root and not only a closed one.
#
...
# as green. The roots go at the top of the report so a reader meets the scope before the count.
#
...
def tree_ref(path):
    ...
    Reachability is answered by asking which remote-tracking ref contains the revision, which is the
    ...
    if not revision:
        _REF_CACHE[top] = None
        return None
    remote = git_say(top, ("for-each-ref", "--contains", revision,
...
def banned_hits(lines, quotations=False, comments=False, path=None, ledger=None):
    ...
    for text, offsets in runs(lines):
        ...
        if quotations:
            ...
            quoted.extend(span.span() for name in NAMED_IN_MARKDOWN
            ...
            for hit in re.finditer(pattern, text, re.IGNORECASE):
                ...
                if tier in exempt:
                    if ledger is not None:
                        ledger.note("the subject is the convention", CONTEXT_REASON,
...
def attributed(note, at, regions):
    ...
    if regions and (at in regions):
        return "%s [generated by %s: fix the generator, then rerun it]" % (note, regions[at])
    return note
...
def walk_markdown(roots, ledger=None):
    ...
    is the one exclusion in this file that stops the read rather than shaping it, because a finding
...
def main():
    ...
    print("  roots configured: %d, %s"
          % (len(roots), "given on the command line" if where_given
             else "this tool's own defaults plus every closed repository found"))
    for one in roots:
        print("    %s%s" % (one.replace("\\", "/"),
                            "" if os.path.exists(one) else "   NOT FOUND"))

    for top, revision, reach, dirty in refs_for(roots):
        print("  measured at %s %s, %s%s"
              % (os.path.basename(top), revision, reach,
    ...
    for path in sorted(walk_markdown(roots, ledger)):
        ...
        wording = banned_tokens(said,
                                quotations=path.endswith(".md"),
                                comments=not path.endswith((".md", ".tex")),
                                path=path, ledger=ledger, regions=regions) + marker_edges(lines)
    ...
    # are the same two states private_survey exists to keep apart.
    print("  excluded: %d" % ledger.total())
    for line in ledger.report():
        print(line)

    if planning:
        print("  --fix is a plan only. Nothing was written, and the rewriting half of it is "
        ...
        for one in manifests_covering(roots):
            print("    this tree carries a signed manifest, %s. After anything writes here: %s"
    ...
    print("  this run read %d file(s) under %d root(s) and nothing outside them. Whether a hook is "
