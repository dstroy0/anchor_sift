...
#
#   python maint/verify_private_sync.py           verify, and say what disagrees
#   python maint/verify_private_sync.py --quiet   only what disagrees
#   python maint/verify_private_sync.py --bypass  answer the gate without satisfying it
#
...
def main():
    ...
    if not hashes:
        out.write("  no %s. Run maint/corpus_manifest.py --write first.\n\n"
                  % " or ".join(MANIFESTS))
        out.flush()
        return 0 if bypassing else 2
