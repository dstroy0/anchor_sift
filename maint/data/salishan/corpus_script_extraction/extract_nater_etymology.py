...
#
#   Usage:  python maint/data/salishan/corpus_script_extraction/extract_nater_etymology.py
#
...
# language of its own fourth column, and that gives a reconstruction in that column its owner.
...
# is listed. A row without one is still a row, and the number is not a reliable row test on its
# own. What it is reliable for is order: the numbers never go backwards, and a number below the
...
# A wrapped cognate instead of a row of its own: the citation or the rest of a gloss that did not
