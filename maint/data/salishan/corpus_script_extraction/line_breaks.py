...
# transformation first. Without that, a join the reader makes and the check does not would report
# every welded word as a hole.
...
# wa above wi above yu, and four characters separate those from gr above eet -?.
JOIN_FLOOR = 4
...
def joined(lines):
    ...
    Takes the lines with their leading whitespace, because that shows whether a break is
    inside a word or between two columns. Returns the repaired lines trimmed, and the welds as
    (line number, fragment, what followed it), which lets a caller report every join it took.
    """
    held = []
    welds = []
    at = 0
