...
#
#   Usage:  python maint/data/salishan/corpus_script_extraction/extract_hall_ctr.py
#
...
# morpheme sign and an occasional subscript. Only the bracketed forms reach the pure stream, the
...
# A surface form. The paper sets every one of them in square brackets, and the only other bracketed
# thing in the paper is a gloss label inside one, which carries no character of the language.
SURFACE = re.compile(r"\[([^\[\]\s]+)\]")

# An underlying form. Slashes with no space between them, which keeps this off the lone
