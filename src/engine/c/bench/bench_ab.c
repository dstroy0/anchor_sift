 ...
 *       which is one pass. That file can be far larger and needs to be: a claim about what every
 *       language does is not testable on a few hundred words.
 */
 ...
 *
 * @note Vocabulary grows with text length. Counting distinct units over whole corpora of different
 ...
 * @note The reference. Its occurrence count is what every other algorithm here is checked against.
 *       The routine is written to be obviously right instead of to be quick.
 */
 ...
 * @param[in] needle_len Bytes in it.
 * @return               Reads performed and occurrences found.
 * @note Reads every corpus cell exactly once and never goes back, which fixes its read count at the
 *       corpus length whatever the data does. That makes it the flat line the other algorithms are
 *       read against: anything above it is doing worse than one pass, and anything below it is
 *       skipping cells that KMP is obliged to look at.
 */
                                 ...
                                 size_t rare)
{
    ...
    while ((start + needle_len) <= corpus_len)
    {
        ...
        // One read at the rarest byte before the full compare. It costs a read whenever the last
 ...
 * @warning This reads the whole corpus's frequencies before searching, which makes it a ceiling on
 *          what any anchor rule can do and not a proposal for how to compute one. What it costs to
 *          learn those frequencies is not counted here.
 */
static size_t ab_best_anchor(const uint8_t *needle, size_t length, const double *frequency)
{
    ...
    for (size_t anchor = 0u; anchor < length; anchor++)
    {
        ...
        // A read that agrees runs a compare and moves on by one
    ...
    while ((start + needle_len) <= corpus_len)
    {
        ...
        if (answer == needle[anchor])
        {
            ...
            while (index < needle_len)
            {
                ...
                {
                    result.reads++;
                    // Bytes read during a compare are answers too
                                 ...
                                 double gain_history, double gain_trend)
{
    ...
    while ((start + needle_len) <= corpus_len)
    {
        ...
        for (size_t offset = 0u; offset < needle_len; offset++)
        {
            ...
            // across many answers. The trend term moves on the error changing, and only that one can
            // act before a drift has finished happening. No amount of averaging produces it, because
            // an average is what a trend is measured against
            if (gain_trend < 0.0)
            {
            ...
            // Bounded to keep a long run of one sign from driving the accumulated term without limit
 ...
 *                        [BORROWS].
 * @return                Reads performed and occurrences found.
 * @note Why the order can be given up. A refutation depends only on the cell that produced it. The
                              ...
                              size_t stride, size_t *depth, unsigned confirm, uint64_t *mirror)
{
    ...
    // Every cell this phase reads is picked before any of them is read. A cell at position p speaks
    // about every alignment covering it, and kills the ones whose needle position holds a different
    // symbol
    ...
    //
    // Checked once over the survivors instead of after every probe. An alignment consistent with the
    // whole probe set is consistent with every prefix of it, because the alive array only ever loses
    // entries, and the per step form costs a factor of the probe count for no further coverage.
    if (mirror != NULL)
    {
        for (size_t start = 0u; start < alignments; start++)
        {
    ...
    // What the probing phase left behind. The model predicts this number, and it is worth seeing
    ...
    *depth = 0u;
    ...
    for (size_t start = 0u; start < alignments; start++)
    {
        ...
        // size as the occurrence set therefore holds the occurrences and no other position.
        // Confirming distinguishes nothing, and it is the step that costs the needle's whole length
        if (confirm == 0u)
        {
            result.found++;
            continue;
 ...
 *       and the survivors are then confirmed. Stopping without confirming is measured in Section 4.9.6
 *       of the research document and is wrong on most searches.
 */
                            ...
                            size_t *probes, uint64_t *mirror)
{
    ...
    *probes = 0u;
    ...
    for (size_t start = 0u; start < alignments; start++)
    {
        ...
        // Every survivor must reproduce every answer already taken, a check the occurrence count
        // cannot make
        if (mirror != NULL)
        {
            for (size_t at = 0u; at < corpus_len; at++)
 ...
 *       with probability $2^{-t H_2}$. The expected count of surviving shifts follows from the corpus
 *       alone. A shift that survives far past that is not a lucky run, it is the corpus repeating,
 *       and the excess is the detection.
 ...
 *       fixes the collision probability, and destroys every relationship between positions, the only
 *       thing a shift can be. Whatever the detector reports there is what it reports on
...
static double ab_spacing(const uint8_t *corpus, size_t length, unsigned *which, double *gap)
{
    ...
    *gap = 0.0;
    ...
    for (unsigned byte = 0u; byte < 256u; byte++)
    {
        ...
        // reason. A candidate below the floor is a ruled line and never a language. Every real
 ...
 * @param[in] corpus     Bytes to examine [BORROWS].
 * @param[in] corpus_len How many.
 * @note The shuffle holds every symbol's count and destroys where they sit, which leaves every
 *       symbol's gaps in it geometric by construction and its dispersion one. Any corpus whose
 *       tightest symbol is far below its shuffle's is marking something.
 */
 ...
 *       between them are not independent. That property defeats every product rule in this document,
 *       and here it works the other way: a run of specific gaps is far less likely than the product
 *       of its symbol rates.
 */
 ...
 * @param[in] corpus_len How many.
 * @param[in] marker     The boundary symbol Section 4.12 found.
 * @note What is being tested. Section 4.12 finds where units end. The spans between markers are
...
static void ab_report(const char *name, const uint8_t *corpus, size_t corpus_len, size_t needle_len)
{
    ...
    // What the corpus emits, and a shipped table can only guess at it
    ...
    // at every step. Any other value is a defect here and never a property of a corpus
    ...
    for (size_t sample = 0u; sample < AB_SAMPLES; sample++)
    {
        ...
        const AbResult arms[12] = {
            ...
            // Amplification for an answer that leads, settling for one that does not, and a
    ...
    // monotone in the stride, since a smaller stride only ever probes more. The largest safe value
 ...
 * @param[in]  length Capacity.
 * @return            Bytes written.
 * @note Each region is filled by the same routine that fills its own corpus. The mixture is the
...
int main(int argc, char **argv)
{
    ...
    // Those two are one pass each, and a real text can be used. The built in corpora are far too
    // small to stand in for one when the question is what every language does
    if (argc > 1)
    {
        ...
        for (int which = 1; which < argc; which++)
        {
            ...
            // any word boundary. The detector would find the formatting and report it as the
