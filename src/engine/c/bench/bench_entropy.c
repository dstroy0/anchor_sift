...
// Sources are drawn from this, and a run reproduces exactly. Held to RFC 6234's vectors by its own
// self test
 ...
 *       where a plug in estimator's bias is largest. It separates the estimators instead of
 *       flattering them.
 */
 ...
 *       so the ladder needs exactly enough rungs to show the trend.
 */
#define MAX_ORDER 5u
 ...
 * @note The uncorrected form at every order. It counts a position as coinciding with itself. The
 *       correction below removes that.
 */
 ...
 *       coincide, the falling factorial in both the numerator and the denominator. Applying
