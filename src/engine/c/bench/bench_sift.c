 ...
 *       A q is followed by a u, and th and he are enormously overrepresented. The claim gets
...
/**
 * @brief English prose, the corpus the english profile was weighted against.
 *
 * @note Ordinary text with ordinary structure. The letter frequencies are Zipf skewed and the
 *       digraphs are correlated. That is the condition the independence question is about.
 */
 ...
 *       every byte equally likely and no correlation between positions. In that regime a weird
 *       anchor does not exist and the multiplication is exact.
 ...
 *       one string is self similar and every needle finds itself, and that artifact already ruined
 *       one measurement here. This varies every field per record. A needle occurs once, and only
 *       the column positions recur.
 ...
 *       binaries can be told apart after the fact.
 */
 ...
 *       applies and no longer means rare, and those policies exist for that arm.
 */
                           ...
                           size_t second_at, size_t needle_len)
{
    ...
    for (size_t start = 0u; (start + needle_len) <= length; start++)
    {
        ...
        // The second anchor is one more compare on a word already in hand. Under SWAR it comes
        // nearly free, and the multiplication below is about that
...
int main(void)
{
    ...
    // against powers of two. That comparison says whether a coprime distance breaks the correlation
    // or the corpus has structure at every distance
