...
static uint64_t cycles_now(void)
{
    ...
    // one arm's loads and stores from crossing them, and the difference between two reads is then
 ...
 * @return              H2 in bits.
 * @note One pass over a histogram, which keeps the dispatch decision cheap enough to take.
 */
