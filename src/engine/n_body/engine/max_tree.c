...
static_assert(ANCHOR_EXACT_LIMBS >= BINOMIAL_BASINS_LIMBS,
...
#else
_Static_assert(ANCHOR_EXACT_LIMBS >= BINOMIAL_BASINS_LIMBS,
               "the exact type must be at least as wide as the residual it is asked about");
...
static void max_tree_imprint(const unsigned int *residual, unsigned int weaker, unsigned int name,
                             unsigned int *key)
{
    const unsigned int *const limbs = &residual[(size_t)weaker * BINOMIAL_BASINS_LIMBS];
    key[0] = ~name;
    for (unsigned int limb = 0u; limb < BINOMIAL_BASINS_LIMBS; limb += 1u)
    {
