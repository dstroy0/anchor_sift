...
STEPS = [
    (
        "read",
        "Read a frame",
        "one frame's voxels come off disk, as the integers the microscope wrote",
        "frame",
    ),
    (
        "basins",
        "Find the cells",
        "each voxel is smoothed twice and the difference kept exactly, then every voxel"
        " steps uphill until it stops: one basin per resting place",
        "frame",
    ),
    (
        "store",
    ...
    (
        "motion",
        "Find the drift",
        "the whole view's shift between two frames, the same count taken at every shift"
        " at once",
        "pair",
    ),
    (
        "overlap",
        "Meet the neighbors",
        "the positive voxels two frames share, basin by basin, at that shift",
        "pair",
    ),
    (
        "climb",
        "Let each cell find its own shift",
        "every cell walks from the view's shift to the shift where it"
        " agrees best with the next frame; each walk stops on its own",
        "pair",
    ),
    (
        "tree",
        "Link and score",
        "the strongest partner each way makes a link, divisions are kept as branches, and"
        " the links are scored against the answer key",
        "pair",
    ),
]
...
NEVER = [
    (
        "It is not trained",
        "there is no model and no learned parameter in the binary: nothing was fitted to this"
        " competition's data or to any other",
    ),
    (
        "It reads nothing else",
        "the only input is the frames of the sample being tracked, and a .cfg of settings."
        " No atlas, no pretrained network, no second dataset, no internet",
    ),
    (
        "It forms no floating point value",
        "every stage is integer arithmetic. The same frames give the same"
...
def read_stages(path):
    ...
    with open(path, encoding="utf-8") as handle:
        for line in handle:
            ...
            found = re.match(
                r"^    (correct link|wrong link|no link made|target among branches|endpoint undetected)\s+(\d+)",
                line,
            )
            if found:
...
def text(x, y, words, size=16, fill=INK, weight="400", anchor="start"):
    return (
        '<text x="%d" y="%d" font-size="%d" fill="%s" font-weight="%s" text-anchor="%s">%s</text>'
...
def wrapped(x, y, words, room, size=15, fill=DIM, step=21):
    ...
    lines.append(line)
    return [
        text(x, y + (step * at), lines[at], size, fill) for at in range(len(lines))
    ], y + (step * len(lines))
...
def main():
    ...
    parser.add_argument(
        "--stages",
        required=True,
        help="a Windows driver log of the 25 samples, no objects written",
    )
    parser.add_argument(
        "--linux", required=True, help="the Linux build's log of the same 25 samples"
    )
    parser.add_argument("--objects", default=os.path.join(ROOT, "view", "data"))
    parser.add_argument(
        "--out", default="D:/kaggle/biohub_cell_tracking/SUBMISSION/infographic.svg"
    )
    ...
    def panel(top, height, title):
        parts.append(
            '<rect x="40" y="%d" width="%d" height="%d" rx="14" fill="%s" stroke="%s"/>'
            % (top, width - 80, height, PANEL, EDGE)
        )
        parts.append(text(64, top + 38, title, 22, INK, "600"))

    parts.append(
        text(40, 70, "Tracking every cell in a developing zebrafish", 36, INK, "700")
    )
    parts.append(
        text(
            40,
            104,
            "One compiled tracker. No training, no model, nothing read but the movie in front of it.",
            18,
            DIM,
        )
    )
    ...
    parts.append(
        text(
            64,
            y + 66,
            "%d training samples, %s frames, one RTX 3070, Windows 11. Each step's bar is its"
            " summed clock over those samples." % (samples, grouped(frames)),
            15,
            DIM,
        )
    )
    ...
    for at in range(len(STEPS)):
        ...
        parts.extend(lines)
        parts.append(
            '<rect x="%d" y="%d" width="%d" height="22" rx="4" fill="%s"/>'
            % (bar_left, row + 2, length, colour)
        )
        parts.append(
            text(
                bar_left + length + 10,
                row + 19,
                "%s ms" % grouped(totals[name]),
                14,
                DIM,
            )
        )
        row += 78
    parts.append(
        '<rect x="64" y="%d" width="16" height="16" rx="3" fill="%s"/>'
        % (row - 4, FRAME)
    )
    parts.append(
        text(
            88,
            row + 9,
            "once per frame: %s ms, %s of the engine time"
            % (grouped(per_frame), share(per_frame, whole)),
            16,
            INK,
        )
    )
    parts.append(
        '<rect x="620" y="%d" width="16" height="16" rx="3" fill="%s"/>'
        % (row - 4, PAIR)
    )
    parts.append(
        text(
            644,
            row + 9,
            "once per pair of frames: %s ms, %s"
            % (grouped(per_pair), share(per_pair, whole)),
            16,
            INK,
        )
    )
    ...
    for title, meaning in NEVER:
        parts.append(
            '<circle cx="76" cy="%d" r="10" fill="none" stroke="%s" stroke-width="3"/>'
            % (row - 5, BAD)
        )
        parts.append(
            '<line x1="69" y1="%d" x2="83" y2="%d" stroke="%s" stroke-width="3"/>'
            % (row - 12, row + 2, BAD)
        )
    ...
    parts.append(
        text(
            64,
            y + 110,
            "%s of %s" % (grouped(correct), grouped(edges)),
            54,
            GOOD,
            "700",
        )
    ...
    parts.append(
        text(
            64,
            y + 144,
            "labeled links correct, %s, strict scoring" % share(correct, edges),
            18,
            INK,
        )
    )
    ...
    for label, colour in (
        ...
        parts.append(
            '<rect x="%d" y="%d" width="%d" height="34" fill="%s"/>'
            % (cursor, y + 72, length, colour)
        )
        cursor += length
    parts.append(
        text(
            stack_left,
            y + 140,
            "wrong %s (%s), no link %s (%s), endpoint not detected %s"
            % (
                grouped(pooled.get("wrong link", 0)),
                share(pooled.get("wrong link", 0), edges),
                grouped(pooled.get("no link made", 0)),
                share(pooled.get("no link made", 0), edges),
                grouped(pooled.get("endpoint undetected", 0)),
            ),
            15,
            DIM,
        )
    ...
    parts.append(
        text(
            64,
            y + 196,
            "The same %s samples on the Linux build: %s, wall clock %s ms."
            % (
                linux_samples,
                (
                    "the same pooled score, edge rows byte for byte"
                    if linux_pooled == pooled
                    else "a pooled score that differs from the Windows build's"
                ),
                grouped(linux_wall),
            ),
            15,
    ...
    parts.append(
        text(
            64,
            y + 70,
            "One sample of %d frames, %d x %d x %d voxels, as one file, including the .cfg that"
            " made it:" % (header[2], header[3], header[4], header[5]),
            16,
            INK,
        )
    )
    ...
    parts.append(
        '<rect x="64" y="%d" width="%d" height="30" rx="4" fill="%s"/>'
        % (y + 90, dense_room, EDGE)
    )
    parts.append(
        text(
            64 + dense_room + 12,
    ...
    parts.append(
        '<rect x="64" y="%d" width="%d" height="30" rx="4" fill="%s"/>'
        % (y + 130, object_length, FRAME)
    )
    parts.append(
        text(
            64 + object_length + 12,
            y + 151,
            "%s bytes as runs, %d times smaller"
            % (grouped(object_bytes), dense // object_bytes),
            15,
            DIM,
        )
    )
    lines, _ = wrapped(
        ...
        % (len(objects), grouped(all_object_bytes)),
        width - 200,
        15,
        INK,
        24,
    ...
    checks = [
        (
            "Runs against voxels",
            "%d objects: every basin's run lengths sum to its voxel count, %d aberrant"
            % (len(objects), aberrant),
        ),
        (
            "Settings against settings",
            "a .cfg written by a run, run again, writes the same bytes and the same edges",
        ),
    ]
    row = y + 78
    ...
    parts.append(
        text(
            40,
            y + 10,
            "Membrane thickness: Bionumbers, cell biology by the numbers; bilayer 3.6 to 4.3 nm"
            " in rat hepatocyte plasma membranes (Mitra et al., PNAS 2004).",
            12,
            DIM,
        )
    )
    ...
    svg = (
        [
            '<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 %d %d" font-family="Segoe UI,'
            ' Roboto, Helvetica, Arial, sans-serif">' % (width, height, width, height),
            '<rect width="%d" height="%d" fill="%s"/>' % (width, height, PAGE),
        ]
        + parts
        + ["</svg>"]
    )
    ...
    print(
        "  %d samples, %d frames, %d of %d ms per frame, %d of %d edges correct, %d objects, %d aberrant"
